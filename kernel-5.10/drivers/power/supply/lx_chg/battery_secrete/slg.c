...
#if IS_ENABLED(CONFIG_BATT_VERIFY_SLG_V1)
#define ECC_SIGN_WAIT_MAX           80
#define GENERAL_WAIT_MAX            10
#endif
#if IS_ENABLED(CONFIG_BATT_VERIFY_SLG_V2)
#define ECC_SIGN_WAIT_MAX           280
#define GENERAL_WAIT_MAX            8
...
static int bus_send_recv(unsigned char *input, int input_len,
			 unsigned char *output, size_t output_size, int *output_len)
{
	...
	if (len != 0) {
		...
		if (recv_crc == calc_crc) {
			...
			if (likely(output_size >= len)) {
				memcpy(output, &recv[1], len);
				*output_len = len;
			} else {
				ret = RECV_LENGTH_ERROR;
				DEBUG("w1 output_size = %zu, len = %d\n",
					output_size, len);
			}
	...
	if (((ret == RECV_CRC_ERROR) || (ret == RECV_LENGTH_ERROR))
	    && (retry > 0)) {
		printk("[slg] command send failed ret = %d, retry %d\n",
		       ret, 5 - retry + 1);
		retry--;
		goto RETRY;
	}
...
int slg_memory_write(int index, const unsigned char *page, int pageSize)
{
    ...
    ret = bus_send_recv(send, 0x23, recv, sizeof(recv), &recv_len);
...
int slg_memory_read(int index, unsigned char *output, int *output_len)
{
	...
	*output_len = 0;
	...
	memset(recv, 0xFF, sizeof(recv));
	ret = bus_send_recv(send, 3, recv, sizeof(recv), &recv_len);
		   ...
		   unsigned char *output, int *output_len)
{
	...
	*output_len = 0;
	...
	memset(recv, 0xFF, sizeof(recv));
	ret = bus_send_recv(send, index, recv, sizeof(recv), &recv_len);
...
int slg_dc_get(void)
{
	...
	memset(recv, 0xFF, sizeof(recv));
	ret = bus_send_recv(send, 2, recv, sizeof(recv), &recv_len);
...
int slg_poweroff_mode(void)
{
	...
	memset(recv, 0xFF, sizeof(recv));
	ret = bus_send_recv(send, 4, recv, sizeof(recv), &recv_len);
...
static const int PAGE_INDEX1 = 1;
static const int PAGE_INDEX2 = 2;

//CA public key X's page number
static const int CA_PUBLIC_KEY_X = 17;

//CA public key Y's page number
static const int CA_PUBLIC_KEY_Y = 18;

//device signer R's page number
static const int DEVICE_SIGNER_R = 19;

//device signer S's page number
static const int DEVICE_SIGNER_S = 20;

//device public key X's page number
static const int DEVICE_PUBLIC_KEY_X = 21;

//device public key Y's page number
static const int DEVICE_PUBLIC_KEY_Y = 22;
...
static int authenticate_battery(void)
{
	...
	if (batt_id[0] == GY_SN1 || batt_id[0] == GY_SN2)
...
static int slg_get_first_usage_date(struct auth_device *auth_dev, u8 *first_usage_date, int len)
{
	int ret = 0, length = 0, i = 0, invalid_count = 0;
	uint8_t page_data[32];

	ret = slg_memory_read(PAGE_INDEX1, page_data, &length);
	if (ret != 0) {
		pr_err("%s %d failed: %d\n", __func__, __LINE__, ret);
		return -1;
	}

	memcpy(first_usage_date, page_data, len);

	for (i = 0; i < len; i++) {
		if (first_usage_date[i] == 0xff)
			invalid_count++;
	}

	if (invalid_count >= 5) {
		pr_err("%s invalid value, set 0\n", __func__);
		memset(first_usage_date, '0', len);
	}

	pr_info("%s first_usage_date:\n", __func__);
	for (i = 0; i < len; i++) {
		pr_info("%s data[%d]:0x%x\n", __func__, i, first_usage_date[i]);
	}

	return 0;
}

static int slg_set_first_usage_date(struct auth_device *auth_dev, u8 *first_usage_date, int len)
{
	int ret = 0, length = 0, i = 0;
	unsigned char buf[128];

	ret = slg_memory_read(PAGE_INDEX1, buf, &length);
	if (ret != 0) {
		pr_err("%s read fail\n", __func__);
		return ret;
	}
	...
	ret = slg_memory_write(PAGE_INDEX1, buf, 32);
	if (ret != 0) {
		pr_err("%s write fail\n", __func__);
		return ret;
	}

	pr_info("%s first_usage:\n", __func__);
	for (i = 0; i < len; i++) {
...
int slg_get_cycle_count(struct auth_device *auth_dev, u32* cycle_count)
{
	...
	int len;
	int get_count;
	uint8_t device_pubkey[0xFF];

	ret = slg_memory_read(PAGE_INDEX2, device_pubkey, &len);
	if (ret != 0) {
		pr_err("%s read failed: %d\n", __func__, ret);
		return ret;
	}

	get_count = device_pubkey[31] & 0xff;
	if (get_count == 0xff) {
		get_count = 0;
		device_pubkey[31] = 0;
		ret = slg_memory_write(PAGE_INDEX2, device_pubkey, len);
		if (ret != 0) {
	...
	pr_info("%s get_count from memory:%d\n", __func__, get_count);
	if (get_count >= 29) {
		ret = slg_dc_get();
		if (ret == DC_INIT_VALUE)
	...
	pr_info("%s get_count from auth:%d\n", __func__, *cycle_count);
...
int slg_set_cycle_count(struct auth_device * auth_dev, u32 set_cycle_count, u32 get_cycle)
{
	...
	if (set_cycle_count < 30) {
		device_pubkey[31] = set_cycle_count;
		ret = slg_memory_write(PAGE_INDEX2, device_pubkey, len);
		if (ret != 0) {
			pr_err("%s write fail\n", __func__);
			return ret;
...
static int slg_get_ui_soh(struct auth_device *auth_dev, u8 *ui_soh_data, int len)
{
	...
	ret = slg_memory_read(PAGE_INDEX2, page_data, &length);
	if (ret != 0) {
		pr_err("%s %d failed: %d\n", __func__, __LINE__, ret);
		return ret;
	}
...
static int slg_set_ui_soh(struct auth_device *auth_dev, u8 *ui_soh_data, int len, int raw_soh)
{
	...
	ret = slg_memory_read(PAGE_INDEX2, buf, &length);
	if (ret != 0) {
		pr_err("%s read fail\n", __func__);
		return ret;
	}
	...
	ret = slg_memory_write(PAGE_INDEX2, buf, 32);
	if (ret != 0) {
		pr_err("%s write fail\n", __func__);
		return ret;
	}
