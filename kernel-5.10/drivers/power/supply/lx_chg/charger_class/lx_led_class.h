...
#ifndef __LINUX_LIXUN_LED_CLASS_H__
#define __LINUX_LIXUN_LED_CLASS_H__
...
struct subpmic_led_ops {
	...
	int (*set_led_torch_enable)(struct subpmic_led_dev *subpmic_led,enum SUBPMIC_LED_ID index, bool en, int flashmode);
#else
...
#if IS_ENABLED(CONFIG_IS_P7_PROJECT)
int subpmic_camera_set_led_torch_enable(struct subpmic_led_dev *subpmic_led,enum SUBPMIC_LED_ID index, bool en, int flashmode);
#else
int subpmic_camera_set_led_torch_enable(struct subpmic_led_dev *subpmic_led,enum SUBPMIC_LED_ID index, bool en);
#endif

#endif /* __LINUX_LIXUN_LED__CLASS_H__ */
