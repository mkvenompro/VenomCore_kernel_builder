...
int subpmic_camera_set_led_torch_enable(struct subpmic_led_dev *subpmic_led, enum SUBPMIC_LED_ID id, bool en, int flashmode)
{
	if (!subpmic_led || !subpmic_led->ops)
		return -EINVAL;
	if (subpmic_led->ops->set_led_torch_enable == NULL)
		return -EOPNOTSUPP;
	return subpmic_led->ops->set_led_torch_enable(subpmic_led, id, en, flashmode);
}
...
MODULE_DESCRIPTION("LiXun Led Class Core");
