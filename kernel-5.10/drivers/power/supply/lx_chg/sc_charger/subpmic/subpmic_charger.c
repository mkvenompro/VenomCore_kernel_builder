...
#include "../../charger_class/lx_led_class.h"
...
static int subpmic_chg_write_byte(struct subpmic_chg_device *sc, u8 reg, u8 val)
{
	u8 temp = val;
...
static int subpmic_chg_read_byte(struct subpmic_chg_device *sc, u8 reg, u8 *val)
{
...
/*****************************************************************************/
__maybe_unused
int subpmic_chg_set_adc_func(struct subpmic_chg_device *sc, int channel, bool en)
{
	int ret;
	u8 val[2] = {0};
...
__maybe_unused
static int subpmic_chg_get_adc(struct subpmic_chg_device *sc, enum sc6601_adc_ch channel)
{
	u32 reg = SUBPMIC_REG_HK_IBUS_ADC + channel * 2;
	u8 val[2] = {0};
	u32 ret = 0;
...
static void qc_detect_workfunc(struct work_struct *work)
{
	...
	if (subpmic_chg_hvdcp_detect(sc) < 0) {
		subpmic_chg_field_write(sc, F_QC_EN, false);
		lx_err("subpmic_chg_hvdcp_detect < 0 !!\n");
		goto out;
	}
	...
	if (sc->qc3_support == NO_SUPPORT) {
		do {
			...
			vbus = subpmic_chg_get_adc(sc, SC6601_ADC_VBUS) / 1000;
			if (vbus > 7200) {
...
out:
	...
	lxchg_psy_updata(BUCK_EVENT_HVDCP_DONE);
...
__maybe_unused
static int subpmic_chg_set_qc_term_vbus(struct subpmic_chg_device *sc)
{
	...
	*chg_status = 0;
	...
	case SUBPMIC_CHG_STATE_TERM:
		*chg_status = POWER_SUPPLY_STATUS_FULL;
		subpmic_chg_set_qc_term_vbus(sc);
		break;
	default:
		break;
	}
...
__maybe_unused
static int subpmic_chg_get_chip_chg_status(struct subpmic_chg_device *sc,
					uint32_t *chg_status)
{
	int state = 0;
...
static int subpmic_chg_set_input_curr_lmt(struct subpmic_chg_device *sc, int ma)
{
	...
	lx_info("subpmic_chg_set_input_curr_lmt = %d\n", ma);

	return subpmic_chg_field_write(sc, F_IINDPM, ma);
}

static int subpmic_chg_disable_power_path(struct subpmic_chg_device *sc, bool enable)
{
	return subpmic_chg_field_write(sc, F_PERFORMANCE_EN, enable);
}
...
__maybe_unused
static int subpmic_chg_enable_chg(struct subpmic_chg_device *sc, bool en)
{
...
static int subpmic_chg_set_otg(struct subpmic_chg_device *sc, bool en)
{
	...
	ret = subpmic_chg_enable_chg(sc, !en);
	if (ret < 0) {
		return ret;
	}
	...
	do {
		...
		if (ret < 0) {
			subpmic_chg_enable_chg(sc, true);
			return ret;
		}
...
static irqreturn_t subpmic_chg_hk_alert_handler(int irq, void *data)
{
	...
	if (last_online && !sc->state.online) {
		...
		lxchg_notifier_call_chain(BUCK_EVENT_BC12_DONE, NULL);
	}
...
__maybe_unused
static int subpmic_set_led_torch_enable(struct subpmic_led_dev *led_dev, enum SUBPMIC_LED_ID id, bool en, int flashmode)
{
	struct subpmic_chg_device *sc = subpmic_led_get_private(led_dev);
...
static int subpmic_chg_hw_init(struct subpmic_chg_device *sc)
{
	...
	ret |= subpmic_chg_enable_chg(sc, true);
	...
	/*ret |= subpmic_chg_write_byte(sc, 0xE7, 0x78);*/
	if (!sc->tcpc)
	...
	ret |= subpmic_chg_set_otg_curr(sc, sc->buck_init.iboost);
	ret |= subpmic_chg_set_otg_volt(sc, sc->buck_init.vboost);
			...
			enum chg_adc_channel channel, uint32_t *value)
{
	...
	adc_val = subpmic_chg_get_adc(sc, to_sc6601_adc(channel));
...
static int sc_chg_get_chip_chg_status(struct charger_dev *charger, uint32_t *status)
{
	...
	return subpmic_chg_get_chip_chg_status(sc, status);
}
...
static int sc_chg_disable_power_path(struct charger_dev *charger, bool enable)
{
	...
	return subpmic_chg_disable_power_path(sc, enable);
}
...
static int sc_chg_enable_chg(struct charger_dev *charger, bool en)
{
	...
	return subpmic_chg_enable_chg(sc, en);
}

static int sc_chg_get_chg(struct charger_dev *charger, bool *en)
{
	struct subpmic_chg_device *sc = charger_get_private(charger);
	*en = subpmic_chg_get_chg(sc);
	return 0;
}
...
static int sc_chg_set_qc_term_vbus(struct charger_dev *charger)
{
	...
	return subpmic_chg_set_qc_term_vbus(sc);
}
...
static void subpmic_chg_shutdown(struct platform_device *pdev)
{
	...
	subpmic_chg_enable_chg(sc, false);
}
