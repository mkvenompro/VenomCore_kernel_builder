...
{
	qc_policy->main_icl_votable = find_votable("MAIN_ICL");
	if (!qc_policy->main_icl_votable) {
		qcm_err("failed to get main_icl_votable\n");
		return false;
	}

	qc_policy->total_fcc_votable = find_votable("TOTAL_FCC");
	...
	switch (qc_policy->sm_state) {
		case QCM_STATE_ENTRY:
			...
			if (qc_policy->sm_status == QCM_SM_EXIT) {
				...
				vote(qc_policy->main_icl_votable, QC_POLICY_VOTER, true, QCM_MAIN_CHG_ICL);
		...
		case QCM_STATE_EXIT:
			...
			vote(qc_policy->main_icl_votable, QC_POLICY_VOTER, false, 0);
...
{
	if (qc_policy->charger->real_type == VBUS_TYPE_HVDCP_3 || qc_policy->charger->real_type == VBUS_TYPE_HVDCP_3P5)
		return true;
	else
		return false;
}
...
static void qcm_update_charge_status(struct qcomcharger_policy *qc_policy)
{
	...
	qc_policy->input_suspend = !get_effective_result(qc_policy->main_icl_votable);
