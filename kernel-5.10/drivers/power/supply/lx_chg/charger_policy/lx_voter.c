...
int vote(struct votable *votable, const char *client_str, bool enabled, int val)
{
	...
	if ((votable->votes[client_id].enabled == enabled) &&
		...
		lx_debug("%s: %s,%d same vote %s of val=%d\n",
				votable->name,
		  ...
		  bool enabled, int val)
{
	...
	lock_votable(votable);
	lx_info("%s: current overrivote enabled:%d val:%d voted by vote %s\n", votable->name, enabled, val, override_client);
	if (votable->force_active) {
	...
	if (enabled) {
		...
		lx_info("%s: current vote is now %d voted by vote_override %s\n",
...
void vote_clean(struct votable *votable)
{
	...
	for (i = 0; i < votable->num_clients && votable->client_strs[i]; i++) {
		if (votable->votes[i].enabled
			&& 0 == votable->votes[i].value) {
			vote(votable, votable->client_strs[i], false, 0);
		}
	}
}
EXPORT_SYMBOL(vote_clean);
				...
				void *data)
{
	...
	if (debug_root == NULL) {
		...
		if (!debug_root) {
			lx_err("Couldn't create debug dir\n");
			return ERR_PTR(-ENOMEM);
		}
	}

	if (votable_type >= NUM_VOTABLE_TYPES) {
		lx_err("Invalid votable_type specified for voter\n");
		return ERR_PTR(-EINVAL);
	}
	...
	if (!votable->root) {
		lx_err("Couldn't create debug dir %s\n", name);
		kfree(votable->name);
		kfree(votable);
		return ERR_PTR(-ENOMEM);
	}
