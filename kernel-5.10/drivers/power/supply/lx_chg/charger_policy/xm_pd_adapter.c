...
static int pd_get_pdos(struct adapter_device *adapter_dev)
{
	...
	lx_err("tcpm_inquire_pd_source_cap is %d.\n", ret);
	if (ret < 0) {
		for(pd_wait_cnt = 0; pd_wait_cnt < 25; pd_wait_cnt++){
			msleep(20);
			lx_err("retry tcpm_inquire_pd_source_cap is %d. cnt =%d\n", ret, pd_wait_cnt);
			ret = tcpm_inquire_pd_source_cap(adapter->tcpc, &cap);
			if(ret == 0)
				break;
		}
	...
	for (i = 0; i < 7; i++) {
		...
		lx_err("pdo[%d] { received_pdos is %08x, cap.pdos is %08x}\n",
...
APDO_REGAIN:
		...
		for (i = 0; i < pd_cap.nr; i++) {
			...
			lx_err("%d mv:[%d,%d] %d max:%d min:%d type:%d %d\n",
				i, tacap->min_mv[i],
