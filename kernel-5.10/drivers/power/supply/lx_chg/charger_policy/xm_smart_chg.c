...
static void smart_chg_navigaition_discharge_func(struct charger_manager *manager)
{
	...
	if(!navigaition_ctrl && manager->smart_charge[SMART_CHG_NAVIGATION].en_ret && manager->soc >= manager->smart_charge[SMART_CHG_NAVIGATION].func_val) {
		lx_info("disable charge\n");
		manager->smart_charge[SMART_CHG_NAVIGATION].active_status = true;
		disable_chg_comm_ctrl(manager, NAVIGAITION_VOTER, true);
#if IS_ENABLED(CONFIG_MIEV)
		xmdfs_notifier_call_chain(CHG_DFX_SMART_NAVI_TRIG, NULL);
#endif
	} else if(navigaition_ctrl && (!manager->smart_charge[SMART_CHG_NAVIGATION].en_ret || manager->soc <= (manager->smart_charge[SMART_CHG_NAVIGATION].func_val - 5))) {
		lx_info("enable charge\n");
		manager->smart_charge[SMART_CHG_NAVIGATION].active_status = false;
		disable_chg_comm_ctrl(manager, NAVIGAITION_VOTER, false);
	}
}
...
static void smart_chg_endurance_pro_handler(struct charger_manager *manager)
{
	...
	if(!endurance_ctrl && manager->smart_charge[SMART_CHG_ENDURANCE_PRO].en_ret && manager->soc >= manager->smart_charge[SMART_CHG_ENDURANCE_PRO].func_val) {
		lx_info("disable charge\n");
		manager->smart_charge[SMART_CHG_ENDURANCE_PRO].active_status = true;
		disable_chg_comm_ctrl(manager, ENDURANCE_VOTER, true);
#if IS_ENABLED(CONFIG_MIEV)
		xmdfs_notifier_call_chain(CHG_DFX_SMART_ENDURA_TRIG, NULL);
#endif
	} else if(endurance_ctrl && (!manager->smart_charge[SMART_CHG_ENDURANCE_PRO].en_ret || manager->soc <= (manager->smart_charge[SMART_CHG_ENDURANCE_PRO].func_val - 5))) {
		lx_info("enable charge\n");
		manager->smart_charge[SMART_CHG_ENDURANCE_PRO].active_status = false;
		disable_chg_comm_ctrl(manager, ENDURANCE_VOTER, false);
	}
}
...
static void smart_chg_low_fast_handler(struct charger_manager *manager)
{
	...
	lx_info("soc = %d, thermal_level = %d, thermal_board_temp = %d, pd_active = %d, low_fast_plugin_flag = %d, low_fast_enable = %d, screen_state = %d, b_flag = %d\n", 
                manager->soc, manager->system_temp_level, manager->thermal_board_temp, manager->pd_active, manager->low_fast_plugin_flag, manager->smart_charge[SMART_CHG_LOW_FAST].active_status, 
                manager->sm.screen_state, manager->b_flag);
	if ((manager->pd_active == CHARGE_PD_PPS_ACTIVE) && (manager->soc <= 40) && (manager->low_fast_plugin_flag) && manager->smart_charge[SMART_CHG_LOW_FAST].en_ret) {
		...
		if (fast_flag) {  //stay fast strategy
			...
			if ((manager->soc > 38) && (manager->thermal_board_temp > 380)){
				...
				lx_err("stay fast but cool down, manager->thermal_board_temp = %d, manager->low_fast_ffc = %d\n", manager->thermal_board_temp, manager->low_fast_ffc);
			} else if ((manager->soc > 30) && (manager->thermal_board_temp > 400)){
				if(manager->low_fast_ffc >= 3500){
					manager->low_fast_ffc = 3500;
				}
				lx_err("stay fast but decrease 3.3, manager->thermal_board_temp = %d, manager->low_fast_ffc = %d\n", manager->thermal_board_temp, manager->low_fast_ffc);
			}
#endif
...
static void smart_chg_outdoor_charge_handler(struct charger_manager *manager)
{
	if (manager->smart_charge[SMART_CHG_OUTDOOR_CHARGE].en_ret) {
		...
		lx_info("set smart_charge[SMART_CHG_OUTDOOR_CHARGE].en_ret = %d, enable outdoor_charge\n",
...
void smart_chg_night_charging(struct charger_manager *manager)
{
	...
	lx_info("night_charging = %d, soc = %d, limit = %d\n", manager->night_charging, manager->soc, night_charging_limit);
	if (!night_charging_limit && manager->night_charging && manager->soc >= 80) {
		lx_info("disable charge\n");
		disable_chg_comm_ctrl(manager, NIGHT_CHARGING_VOTER, true);
	} else if(night_charging_limit && (!manager->night_charging || manager->soc <=75)) {
		lx_info("enable charge\n");
		disable_chg_comm_ctrl(manager, NIGHT_CHARGING_VOTER, false);
	}
}
...
static void monitor_fv_overvoltage(struct charger_manager *manager)
{
	...
	fcc_vote = get_effective_result(manager->total_fcc_votable);
	fv_vote = get_effective_result(manager->fv_votable);
	lx_err("%d:%d:%d:%d:%d:%d\n", manager->fv_overvoltage_flag, fv_overvoltage_count, manager->vbat, fv_vote, manager->pd_active, fcc_vote);

	if((manager->vbat > fv_vote) && (manager->pd_active == CHARGE_PD_PPS_ACTIVE) && (!manager->fv_overvoltage_flag)) {
