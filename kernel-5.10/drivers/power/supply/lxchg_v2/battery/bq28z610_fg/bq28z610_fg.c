...
static int __fg_read_byte(struct i2c_client *client, u8 reg, u8 *val)
{
	...
	if(ret < 0)
	{
		lx_info("i2c read byte failed: can't read from reg 0x%02X faild\n", reg);
		return ret;
	}
...
static int fg_read_word(struct bq_fg_chip *bq, u8 reg, u16 *val)
{
	...
	if (ret) {
		lx_info("%s I2C failed to read 0x%02x\n", bq->log_tag, reg);
		return ret;
	}
...
static int fg_read_block(struct bq_fg_chip *bq, u8 reg, u8 *buf, u8 len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (ret) {
			lx_info("%s I2C failed to read 0x%02x\n", bq->log_tag, reg + i);
#if IS_ENABLED(CONFIG_MIEV)
		xmdfs_notifier_call_chain(CHG_DFX_FG_I2C_ERR, NULL);
#endif
			return ret;
		}
		buf[i] = data;
...
static int fg_write_block(struct bq_fg_chip *bq, u8 reg, u8 *data, u8 len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (ret) {
			lx_err("%s I2C failed to write 0x%02x\n", bq->log_tag, reg + i);
#if IS_ENABLED(CONFIG_MIEV)
			xmdfs_notifier_call_chain(CHG_DFX_FG_I2C_ERR, NULL);
#endif
			return ret;
...
static int fg_mac_write_block(struct bq_fg_chip *bq, u16 cmd, u8 *data, u8 len)
{
	...
	if (ret < 0) {
		lx_err("%s failed to write block\n", bq->log_tag);
		return ret;
	}
...
retry:
	...
	if (ret < 0) {
		if (!retry) {
			...
			lx_err("%s failed to read TBAT\n", bq->log_tag);
#if IS_ENABLED(CONFIG_MIEV)
			xmdfs_notifier_call_chain(CHG_DFX_BATT_LINKER_ABSENT, NULL);
#endif
			tbat = 2980;
			if(bq->i2c_error_count < 10)
				bq->i2c_error_count++;
...
retry:
	...
	if (ret) {
		if (!retry) {
			...
			lx_err("%s failed to read cell voltage\n", bq->log_tag);
...
retry:
	...
	if (ret < 0) {
		if (!retry) {
			...
			lx_err("%s failed to read AVG_IBAT\n", bq->log_tag);
			avg_ibat = 0;
			if(bq->i2c_error_count < 10)
				bq->i2c_error_count++;
		}
...
retry:
	...
	if (ret < 0) {
		if (!retry) {
			...
			fcc = pre_fcc;
#endif
			lx_err("%s failed to read FCC,FCC=%d\n", bq->log_tag, fcc);
			if(bq-> i2c_error_count < 10)
				bq->i2c_error_count++;
		}
	}
...
retry:
	...
	if (ret < 0) {
		if (!retry) {
			...
			rm = 708;
#endif
			lx_err("%s failed to read RM,RM=%d\n", bq->log_tag, rm);
			if(bq->i2c_error_count < 10)
				bq->i2c_error_count++;
		}
	}
...
static int fuel_guage_get_rsoc(struct fuel_gauge_dev *fuel_gauge)
{
	struct bq_fg_chip *bq = fuel_gauge_get_private(fuel_gauge);
...
static int fg_set_fastcharge_mode(struct bq_fg_chip *bq, bool enable)
{
	...
	if (enable) {
		...
		lx_info("%s write 3e fastcharge = %d success\n", bq->log_tag, ret);
		if (ret) {
			lx_err("%s failed to write fastcharge = %d\n", bq->log_tag, ret);
			return ret;
		}
	} else {
		ret = fg_mac_write_block(bq, FG_MAC_CMD_FASTCHARGE_DIS, data, 2);
		lx_info("%s write 3f fastcharge = %d success\n", bq->log_tag, ret);
		if (ret) {
			lx_err("%s failed to write fastcharge = %d\n", bq->log_tag, ret);
			return ret;
		}
...
static int calc_delta_time(ktime_t time_last, int *delta_time)
{
	...
	*delta_time = ktime_ms_delta(time_now, time_last);
	...
	lx_info("now:%ld, last:%ld, delta:%d\n", time_now, time_last, *delta_time);
	...
	int raw_soc, int batt_soc, int soc)
{
	...
	if (raw_soc > bq->report_full_rsoc) {
		if (raw_soc == 10000 && bq->last_soc < 99) {
			...
			if (soc_changed) {
				...
				lx_info("%s soc increase changed = %d\n", bq->log_tag, soc_changed);
			} else {
				soc = bq->last_soc;
			}
		} else {
		...
		if (raw_soc == 0 && bq->last_soc > 1) {
			...
			soc_changed = min(1, delta_time);
			if (soc_changed) {
				lx_info("%s soc reduce changed = %d\n", bq->log_tag, soc_changed);
				soc = bq->last_soc - soc_changed;
			} else
				soc = bq->last_soc;
		} else {
	...
	if (bq->last_soc != soc) {
		if(abs(soc - bq->last_soc) > 1){
			...
			lx_info("avoid jump soc = %d last = %d soc_change = %d state = %d ,delta_time = %d\n",
			...
			if(status == POWER_SUPPLY_STATUS_CHARGING){
				if(soc > bq->last_soc){
					...
					lx_info("Do not smooth waiting real soc increase here\n");
					soc = bq->last_soc;
				}
				...
				if(soc < bq->last_soc){
					...
					lx_info("Do not smooth waiting real soc decrease here\n");
					soc = bq->last_soc;
				}
			}
		}else{
		...
		int raw_soc, int batt_soc, int batt_temp, int batt_ma)
{
	...
	if (bq->fast_chg && raw_soc >= bq->report_full_rsoc && raw_soc != FG_RAW_SOC_FULL) {
		...
		if (raw_soc > last_raw_soc && soc_changed) {
			...
			last_optimiz_time = ktime_get();
			lx_info("optimiz_soc:%d, last_optimiz_time%ld\n",
	...
	lx_info("batt_ma_avg:%d, batt_ma:%d, cold_smooth:%d, optimiz_soc:%d",
			batt_ma_avg, batt_ma, cold_smooth, optimiz_soc);
	lx_info("delta_time:%d, change_delta:%d, unit_time:%d"
	...
	lx_info("raw_soc:%d batt_soc:%d,last_batt_soc:%d,system_soc:%d",
...
static int bq_battery_soc_smooth_tracking_new(struct bq_fg_chip *bq, int raw_soc, int batt_soc, int batt_ma)
{
	...
	if (!raw_soc) {
        ...
        if (!is_full_votable) {
                lx_err("failed to get is_full_votable\n");
        }else{
                if(charging_status == POWER_SUPPLY_STATUS_FULL){
                        lx_info("smooth_new: report full, is_full_flag = %d\n", is_full_flag);
                        is_full_flag = true;
                        vote(is_full_votable, SMOOTH_NEW_VOTER, true, 0);
                }

                if(is_full_flag && (raw_soc >= 9700)){
                        lx_info("smooth_new: report full, old raw_soc = %d\n", raw_soc);
                        raw_soc = 100000;
                        lx_info("smooth_new: report full, new raw_soc = %d\n", raw_soc);
                } else if(is_full_flag && (raw_soc >= 9500) && bq->tbat < 0){
                        lx_info("smooth_new: report full, old raw_soc = %d tbat = %d\n", raw_soc, bq->tbat);
                        raw_soc = 100000;
                        lx_info("smooth_new: report full, new raw_soc = %d tbat = %d\n", raw_soc, bq->tbat);
                } else {
                        is_full_flag = false;
                        vote(is_full_votable, SMOOTH_NEW_VOTER, false, 0);
                }
                lx_info("smooth_new:is_full_flag = %d\n", is_full_flag);
        }
#endif
	...
	if(raw_soc >= bq->report_full_rsoc)
        ...
        lx_info("smooth_new: fisrt step, system_soc = %d\n", system_soc);
		...
		!bq->rm && bq->tbat < 150 && last_system_soc >= 1) 
	{
		...
		lx_info("enter low temperature smooth unit_time=%d batt_ma_avg=%d\n", unit_time, batt_ma_avg);
	}
...
#endif
	{
		...
		lx_info("fg jump smooth soc_changed=%d\n", soc_changed);
	}
	...
	if (((charging_status == POWER_SUPPLY_STATUS_DISCHARGING) && (system_soc > last_system_soc)) || ((charging_status == POWER_SUPPLY_STATUS_CHARGING) && (system_soc < last_system_soc) && (ibat_pos_count < 3) && ((time.tv_sec > 10))))
		system_soc = last_system_soc;
	lx_info("smooth_new:sys_soc:%d last_sys_soc:%d soc_delta:%d charging_status:%d unit_time:%d batt_ma_avg=%d\n" ,
	...
	if ((system_soc == 0) && ((bq->vbat >= 3400) || ((time.tv_sec <= 10)))) {
		system_soc = 1;
		lx_err("uisoc::hold 1 when volt > 3400mv. \n");
	}
...
static int fg_set_shutdown_mode(struct bq_fg_chip *bq)
{
	...
	lx_info("%s fg_set_shutdown_mode\n", bq->log_tag);
	...
	if (ret)
		lx_err("%s failed to send shutdown cmd 0\n", bq->log_tag);

	ret = fg_mac_write_block(bq, FG_MAC_CMD_SHUTDOWN, data, 2);
	if (ret)
		lx_err("%s failed to send shutdown cmd 1\n", bq->log_tag);
...
static bool battery_get_psy(struct bq_fg_chip *bq)
{
	...
	if (!bq->batt_psy) {
		lx_err("%s failed to get batt_psy", bq->log_tag);
		return false;
	}
	return true;
}
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	lx_err("fg_update rsoc=%d, raw_soc=%d, vbat=%d, cycle_count=%d\n", bq->rsoc, bq->raw_soc, bq->vbat, bq->cycle_count);

	if (!battery_get_psy(bq)) {
		lx_err("%s fg_update failed to get battery psy\n", bq->log_tag);
		bq->ui_soc = bq->rsoc;
		return;
	} else {
		#if 0
		...
		if(bq->night_charging && bq->ui_soc > 80 && bq->ui_soc > last_soc){
            bq->ui_soc = last_soc;
            lx_err("last_soc = %d, night_charging = %d,\n", last_soc, bq->night_charging);
        }
...
out:
		lx_info("%s [FG_STATUS] [UISOC RSOC RAWSOC TEMP_SOC SOH] = [%d %d %d %d %d], [VBAT CELL0 CELL1 IBAT TBAT FC FAST_MODE] = [%d %d %d %d %d %d %d]\n", bq->log_tag,
		...
		if (bq->batt_psy && (last_soc != bq->ui_soc || delta_temp > 5 || bq->ui_soc == 0 || bq->rsoc == 0)) {
			lx_err("last_soc = %d, last_temp = %d, delta_temp = %d\n", last_soc, last_temp, delta_temp);
	...
	int *val)
{
	...
	if (ret)
		lx_err("failed to get FG_MAC_CMD_LIFETIME1\n");
	...
	int *val)
{
	...
	{
		lx_err("this Bq_Fg is not support this function.\n");
		return -1;
	}
	...
	if(ret < 0)
	{
		lx_err("read isc_alert_level occur error.\n");
		return ret;
	}
	*val = isc_alert_level;
	lx_err("now isc:%d.\n", *val);
	...
	int *val)
{
	...
	if(ret < 0)
	{
		lx_err("read soa_alert_level occur error.\n");
		return ret;
	}
	*val = soa_alert_level;
	lx_err("now soa:%d.\n", *val);
	...
	int *val)
{
	...
	ret = fg_read_word(gm, FG_MAC_CMD_CHARGING_STATUS, &charging_state);
		if (ret < 0)
			lx_err("failed to read MAC\n");

	*val = !!(charging_state & BIT(3));
	lx_err("charging_state:%d.\n", *val);
	return ret;
}
...
static int fg_init_psy(struct bq_fg_chip *bq)
{
	...
	if (IS_ERR(bq->fg_psy)) {
		lx_err("%s failed to register fg_psy", bq->log_tag);
...
static int fake_fg_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_MODEL_NAME:
		val->strval = "fake-fg";
...
static int fake_fg_init_psy(struct bq_fg_chip *bq)
{
	struct power_supply_config fg_psy_cfg = {};

	bq->fg_psy_d.name = "fake_bms";
	bq->fg_psy_d.type = POWER_SUPPLY_TYPE_UNKNOWN;
	bq->fg_psy_d.properties = fg_props;
	bq->fg_psy_d.num_properties = ARRAY_SIZE(fg_props);
	bq->fg_psy_d.get_property = fake_fg_get_property;
	bq->fg_psy_d.set_property = NULL;
	fg_psy_cfg.supplied_to = mtk_bms_supplied_to;
	fg_psy_cfg.num_supplicants = ARRAY_SIZE(mtk_bms_supplied_to);
	fg_psy_cfg.drv_data = bq;
...
static int fg_parse_dt(struct bq_fg_chip *bq)
{
	...
	bq->enable_shutdown_delay = of_property_read_bool(node, "enable_shutdown_delay");
        ...
        bq->slave_connect_gpio = of_get_named_gpio(node, "slave_connect_gpio", 0);
          lx_err("%s slave_connect_gpio = %d \n", bq->log_tag, bq->slave_connect_gpio );    
          if (!gpio_is_valid(bq->slave_connect_gpio)) {
                 lx_info("failed to parse slave_connect_gpio\n");
                 return -1;
          }
#endif
	...
	if (ret)
		lx_err("%s failed to parse soc_gap_1s\n", bq->log_tag);
	...
	if (size) {
		...
		if (bq->dec_rate_seq) {
			...
			if (bq->dec_rate_len % 2) {
				lx_err("%s invalid soc decimal rate seq\n", bq->log_tag);
				return -EINVAL;
			}
			...
			lx_err("%s error allocating memory for dec_rate_seq\n", bq->log_tag);
		}
	}
...
static int fg_check_device(struct bq_fg_chip *bq)
{
	...
	if (ret) {
		lx_err("failed to get FG_MAC_CMD_DEVICE_NAME\n");
		return ret;
	} else
		lx_err("device name : %s\n", data);
	...
	lx_err("device name2 : %s\n", data);

	if (!strncmp(data, "bq28z610", 8)) {
		bq->device_name = BQ_FG_BQ28Z610;
		strcpy(bq->model_name, "bq28z610");
		strcpy(bq->log_tag, "[LXCHG_BQ28Z610]");
	} else if (!strncmp(data, "xm81@bn7", 8) || 
		      !strncmp(data, "xn10@bn6", 8)) {
		bq->device_name = BQ_FG_MPC8011B;
		strcpy(bq->model_name, "mpc8011b");
		strcpy(bq->log_tag, "[LXCHG_MPC8011B]");
	} else {
		bq->device_name = BQ_FG_UNKNOWN;
		strcpy(bq->model_name, "UNKNOWN");
		strcpy(bq->log_tag, "[LXCHG_UNKNOWN_FG]");
	}

	ret = fg_mac_read_block(bq, FG_MAC_CMD_MANU_NAME, data, 32);
	if (ret)
		lx_info("failed to get FG_MAC_CMD_MANU_NAME\n");
	else
		lx_err("manu name : %s\n", data);

	if (bq->device_name != BQ_FG_UNKNOWN)
		bq->chip_ok = true;
	else
		bq->chip_ok = false;
	...
	if (ret)
		lx_info("failed to get FG_MAC_CMD_DEVICE_CHEM\n");
	else
		lx_err("device chem : %s\n", data);
...
static int fg_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (IS_ERR(bq->regmap)) {
		lx_err("failed to allocate regmap\n");
		return PTR_ERR(bq->regmap);
	}

	ret = fg_check_device(bq);
	if (ret) {
		lx_err(" failed to fg_check_device!\n");
		fake_fg_init_psy(bq);//no use
		return -ENODEV;
	}

	ret = fg_parse_dt(bq);
	if (ret) {
		lx_err("%s failed to parse DTS\n", bq->log_tag);
		return ret;
	}
	...
	ret = fg_init_psy(bq);
	if (ret) {
		lx_err("%s failed to init psy\n", bq->log_tag);
		return ret;
	}

	ret = sysfs_create_group(&bq->dev->kobj, &fg_attr_group);
	if (ret) {
		lx_err("%s failed to register sysfs\n", bq->log_tag);
		return ret;
	}
	...
	if (!bq->fuel_gauge) {
		...
		lx_err("%s failed to register fuel_gauge\n", bq->log_tag);
		return ret;
	}
	...
	hardwareinfo_set_prop(HARDWARE_BMS_GAUGE, "gauge");
	lx_err("HARDWARE_BMS_GAUGE is set!\n");

	lx_info("%s FG probe success\n", bq->log_tag);
...
static void fg_shutdown(struct i2c_client *client)
{
	...
	lx_info("%s bq fuel gauge driver shutdown!\n", bq->log_tag);
}
...
int bq28z610_fg_init(void)
{
	int rc;
	rc = i2c_add_driver(&fg_driver);
	if (rc)
