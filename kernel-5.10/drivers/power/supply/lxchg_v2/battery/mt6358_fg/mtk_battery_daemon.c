	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	auth_dev = get_batt_auth_by_name("secret_ic");
	if (!auth_dev) {
		bm_err("failed to get battery auth device\n");
		goto out;
	}

	ret = auth_device_get_uisoh(auth_dev, ui_soh_data, 11);
	if (ret != 0) {
		memcpy(ui_soh_data, gm->last_ui_soh_data, 11);
		bm_err("get auth ui soh error\n");
	} else
		memcpy(gm->last_ui_soh_data, ui_soh_data, 11);
...
out:
	...
	bm_info("%s: latest_ui_soh = %d\n", __func__, ui_soh_data[0]);

	return ret;
}

static ssize_t ui_soh_store(
	struct device *dev, struct device_attribute *attr,
	const char *buf, size_t size)
{
	struct mtk_battery *gm = get_mtk_battery();
	...
	while ((pchar = strsep(&qchar, " ")))
	{
		...
		ui_soh_data[i] = val;
		val = 0;
		bm_info("%s ui_soh_data[%d]: %d\n", __func__, i, ui_soh_data[i]);
		i++;
	}
	ui_soh = ui_soh_data[0];
	raw_soh = gm->aging_factor/100;

	ret = auth_device_set_uisoh(auth_dev, ui_soh_data, i, raw_soh);
	if (ret != 0) {
		bm_err("save ui soh to auth ic error\n");
		return -1;
	}
	...
	struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_KERNEL_UISOC:
	{
		...
		if (daemon_ui_soc < 100) {
			bm_debug("[%s]Make daemon_ui_soc greater than 100 to close MTK shutdown:%d\n",
				__func__,
				daemon_ui_soc);
			daemon_ui_soc = 100;
		}
	...
	case FG_DAEMON_CMD_SET_SOC:
	{
		...
		#if IS_ENABLED(CONFIG_LIXUN_CHARGER_MANAGER)
		gm->rsoc = int_value;
		#endif
	...
	case FG_DAEMON_CMD_SET_AGING_FACTOR:
	{
		...
		if (gm->fake_aging_factor != 0)
			gm->aging_factor = gm->fake_aging_factor;
		bm_debug("[K]FG_DAEMON_CMD_SET_AGING_FACTOR %d, %d\n",
		gm->aging_factor, gm->fake_aging_factor);
	}
	break;
...
#if IS_ENABLED(CONFIG_LIXUN_CHARGER_MANAGER)
static int gm30_get_soc_decimal_rate(struct fuel_gauge_dev *fuel_gauge)
{
...
static int gm30_get_soc_decimal(struct fuel_gauge_dev *fuel_gauge)
{
	struct mtk_battery *gm = fuel_gauge_get_private(fuel_gauge);
	...
	dec_rate = gm30_get_soc_decimal_rate(fuel_gauge);
	soc = gm->fg_cust_data.ui_old_soc;
...
static int gm30_get_rsoc(struct fuel_gauge_dev *fuel_gauge)
{
	int val;
	struct mtk_battery *gm = fuel_gauge_get_private(fuel_gauge);

	if(IS_ERR_OR_NULL(gm)) {
		bm_err("%s get bms gm fail\n", __func__);
		return -EINVAL;
	}
...
static int gm30_set_rsoc_update0(struct fuel_gauge_dev *fuel_gauge, bool value)
{
	struct mtk_battery *gm = fuel_gauge_get_private(fuel_gauge);
	...
	gm->rsoc_update0 = value;
	return 0;
}

#if IS_ENABLED(CONFIG_LIXUN_SOH2_SUPPORT)
static int gm30_get_soh(struct fuel_gauge_dev *fuel_gauge)
{
	int val;
	struct mtk_battery *gm = fuel_gauge_get_private(fuel_gauge);
...
static int gm30_set_soh(struct fuel_gauge_dev *fuel_gauge, int value)
{
	struct mtk_battery *gm = fuel_gauge_get_private(fuel_gauge);
	...
	bm_err("%s fake_aging_factor value = %d\n", __func__, value);
	gm->fake_aging_factor = value;
	return 0;
}
#endif

static int gm30_get_c_car(struct fuel_gauge_dev *fuel_gauge)
{
	int val;
	struct mtk_battery *gm = fuel_gauge_get_private(fuel_gauge);
	if(IS_ERR_OR_NULL(gm)) {
...
static int gm30_get_v_car(struct fuel_gauge_dev *fuel_gauge)
{
	int val;
	struct mtk_battery *gm = fuel_gauge_get_private(fuel_gauge);
	if(IS_ERR_OR_NULL(gm)) {
	...
	val = gm->fg_cust_data.v_soc * gm->algo_qmax / 10000;
	return val;
}

static struct fuel_gauge_ops fuel_gauge_ops = {
	.get_soc_decimal = gm30_get_soc_decimal,
	.get_soc_decimal_rate = gm30_get_soc_decimal_rate,
	.get_rsoc = gm30_get_rsoc,
	.set_rsoc_update0 = gm30_set_rsoc_update0,
	#if IS_ENABLED(CONFIG_LIXUN_SOH2_SUPPORT)
	.get_soh = gm30_get_soh,
	.set_soh = gm30_set_soh,
	#endif
	.get_c_car = gm30_get_c_car,
	.get_v_car = gm30_get_v_car,
};
#endif
