...
int fgauge_get_profile_id(void)
{
	...
	if (IS_ERR_OR_NULL(auth_dev)) {
		bm_err("get battery auth device fail\n");
		return 0;
	}

	bm_err("%s: battery id  = %d\n", __func__, auth_dev->battery_id);

	return auth_dev->battery_id;
}

static int fgauge_get_batt_auth(void)
{
	struct auth_device *auth_dev;
	...
	bm_err("%s: auth_result  = %d\n", __func__, auth_dev->auth_result);

	return auth_dev->auth_result;
}

int fgauge_get_cycle_count(void)
{
	struct auth_device *auth_dev;
	u32 cycle_count = 0;
	int ret = 0;
	...
	ret = auth_device_get_cycle_count(auth_dev, &cycle_count);
	if (ret != 0) {
		bm_err("read auth cycle count error\n");
		return 0;
	}
	bm_err("%s: cycle count  = %d\n", __func__, cycle_count);
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		val->intval = gm->ui_soc * (gm->algo_qmax * gm->aging_factor / 100) / 100;
		if (val->intval > CHARGER_FULL_MAX)
			val->intval = CHARGER_FULL_MAX;
		break;
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		val->intval = fgauge_get_batt_auth();
		break;
	default:
		ret = -EINVAL;
		break;
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		...
		if (!online.intval) {
			...
			if (status.intval == POWER_SUPPLY_STATUS_NOT_CHARGING) {
				...
				#if IS_ENABLED(CONFIG_LIXUN_CHARGER_MANAGER)
				dv2_chg_psy = power_supply_get_by_name("usb");
				#else
...
void fg_custom_init_from_header(struct mtk_battery *gm)
{
	...
	if (gm->gauge != NULL) {
		gm->gauge->hw_status.vbat2_det_time = VBAT2_DET_TIME;
		gm->gauge->hw_status.vbat2_det_counter = VBAT2_DET_COUNTER;
		bm_err("vbat2_det_time, vbat2_det_counter: %d %d",
			gm->gauge->hw_status.vbat2_det_time, gm->gauge->hw_status.vbat2_det_counter);
	}
	...
	struct mtk_battery *gm)
{
	...
	cycle_count = fgauge_get_cycle_count();
	...
	for (i = 0; i < MAX_TABLE; i++) {
		...
		if (cycle_count <= 100) {
			...
			fg_read_dts_val_by_idx(np, "g_FG_PSEUDO100_100_300_cycle",
				i*TOTAL_BATTERY_NUMBER + gm->battery_id,
				&(fg_table_cust_data->fg_profile[i].pseudo100),
				UNIT_TRANS_100);
		} else if (cycle_count <= 800) {
			fg_read_dts_val_by_idx(np, "g_FG_PSEUDO100_300_800_cycle",
				i*TOTAL_BATTERY_NUMBER + gm->battery_id,
				&(fg_table_cust_data->fg_profile[i].pseudo100),
				UNIT_TRANS_100);
		} else {
			fg_read_dts_val_by_idx(np, "g_FG_PSEUDO100_ov_800_cycle",
				i*TOTAL_BATTERY_NUMBER + gm->battery_id,
				&(fg_table_cust_data->fg_profile[i].pseudo100),
				UNIT_TRANS_100);
		}
	...
	for (i = 0; i < fg_table_cust_data->active_table_number; i++) {
		...
		if (cycle_count < 400) {
			...
			sprintf(profile_name, "battery%d_profile_t%d", bat_id, i);
		} else if (cycle_count > 800) {
			sprintf(node_name, "battery800cc%d_profile_t%d_col", bat_id, i);
			sprintf(profile_name, "battery800cc%d_profile_t%d", bat_id, i);
		} else {
					sprintf(node_name, "battery400cc%d_profile_t%d_col", bat_id, i);
			sprintf(profile_name, "battery400cc%d_profile_t%d", bat_id, i);
		}
		...
		bm_err("%s, node_name:%s,column:%d",
				__func__, node_name, column);
		bm_err("%s, profile_name:%s",
				__func__, profile_name);
		
		fg_custom_parse_table(gm, np, profile_name,
	...
	int *val)
{
	if (gm->fake_aging_factor != 0)
		*val = gm->fake_aging_factor;
	else
		*val = gm->aging_factor/100;
	return 0;
}

static int soh_set(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int val)
{
	gm->fake_aging_factor = val;
	return 0;
}
...
static struct mtk_battery_sysfs_field_info battery_sysfs_field_tbl[] = {
	...
	BAT_SYSFS_FIELD_RW(soh, BAT_PROP_SOH),
};
...
static int shutdown_event_handler(struct mtk_battery *gm)
{
	...
	gm->charger = charger_find_dev_by_name("primary_chg");
	if (!gm->charger)
		bm_err("failed to master charge device\n");
	...
	if (sdd->shutdown_status.is_soc_zero_percent) {
		if (current_ui_soc == 0) {
			...
			if (tmp_duraction.tv_sec >= SHUTDOWN_TIME) {
				...
				//disable otg before shutdown
				if (gm->charger != NULL)
					charger_set_otg(gm->charger, false);
				kernel_power_off();
