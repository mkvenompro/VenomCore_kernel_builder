...
stick_ReturnCode_t stickapi_write_page(u8* user_date, u8 page_index, u8 len);
stick_ReturnCode_t stickapi_read_page(u8* user_date,u8 page_index, u8 len);
stick_ReturnCode_t stickapi_read_dc_value(u32 *cycle_count);
stick_ReturnCode_t stickapi_dc_decrease(void);
...
static int st_get_battery_id(struct auth_device *auth_dev, u8 *bat_id)
{
	...
	if (page0_data[0] == GY_SN1 || page0_data[0] == GY_SN2)		//GY--first_supplier
		*bat_id = BATTERY_VENDOR_FIRST;
	else if (page0_data[0] == NVT_SN1 && page0_data[1] == NVT_SN2)  //NVT--second_supplier
...
static int st_get_first_usage_date(struct auth_device *auth_dev, u8 *first_usage_date, int len)
{
	int ret = 0, i = 0, invalid_count = 0;
	uint8_t page6_data[16] = {0};
	u8 retry_cnt = 10;
	u8 exit_retry_cnt = 10;

	mutex_lock(&g_sd->mutex);
	while (retry_cnt) {
		ret = stick_exit_hibernate_and_reset(g_sd);
		if (ret == ST1WIRE_OK) {
			mutex_unlock(&g_sd->mutex);
			break;
		}

		pr_err("%s cnt:%d, exit hibernate error: %d\n", __func__, retry_cnt, ret);
		retry_cnt--;
		msleep(20);
	}
	...
	ret = stickapi_read_page(page6_data, PAGE_INDEX6, 16);
	if (ret != 0) {
		pr_err("%s read fail\n", __func__);
		return ret;
	}
	...
	if (len > 16) {
		pr_err("%s %d failed: len outof size\n", __func__, __LINE__);
		return ST1WIRE_ACK_ERROR;
	}

	memcpy(first_usage_date, page6_data, len);
	...
	pr_info("%s get first_usage_date:\n", __func__);
	for (i = 0; i < len; i++) {
		pr_info("%s data[%d]:0x%x\n", __func__, i, first_usage_date[i]);
	}

	mutex_lock(&g_sd->mutex);
	while (exit_retry_cnt) {
		ret = stick_set_hibernate(g_sd);
		if (ret == ST1WIRE_OK) {
			mutex_unlock(&g_sd->mutex);
			break;
		}

		pr_err("%s cnt:%d, enter hibernate error: %d\n", __func__, retry_cnt, ret);
		exit_retry_cnt--;
		msleep(20);
	}
...
static int st_set_first_usage_date(struct auth_device *auth_dev, u8 *first_usage_date, int len)
{
	int ret = 0, i = 0;
	u8 page6_data[16] = {0};
	u8 retry_cnt = 10;
	...
	for (i = 0; i < len; i++)
		page6_data[i] = first_usage_date[i];

	ret = stickapi_write_page(page6_data, PAGE_INDEX6, 16);
	if (ret != 0) {
		pr_err("%s write fail\n", __func__);
		return ret;
	}
	...
	pr_info("%s set first_usage_date:\n", __func__);
	for (i = 0; i < len; i++) {
		pr_info("%s page6_data[%d]:0x%x\n", __func__, i, page6_data[i]);
	}
...
static int st_get_ui_soh(struct auth_device *auth_dev, u8 *ui_soh_data, int len)
{
	...
	ret = stickapi_read_page(ui_soh, PAGE_INDEX8, 16);
	if (ret != 0) {
		pr_err("%s read fail\n", __func__);
		return ret;
	}

	ret = stickapi_read_page(&ui_soh[16], PAGE_INDEX9, 16);
	if (ret != 0) {
		pr_err("%s read fail\n", __func__);
		return ret;
	}
...
static int st_set_ui_soh(struct auth_device *auth_dev, u8 *ui_soh_data, int len, int raw_soh)
{
	...
	for (i = 0; i < len; i++) {
		ui_soh[i] = ui_soh_data[i];
	}
	ui_soh[15] = raw_soh;

	ret = stickapi_write_page(ui_soh, PAGE_INDEX8, 16);
	if (ret != 0) {
		pr_err("%s write fail\n", __func__);
		return ret;
	}

	ret = stickapi_write_page(&ui_soh[16], PAGE_INDEX9, 16);
	if (ret != 0) {
		pr_err("%s write fail\n", __func__);
		return ret;
	}
...
int st_get_cycle_count(struct auth_device *auth_dev, u32* cycle_count)
{
	u32 get_count = 0;
	u32 get_dc_value = 0x1ffff;
	u8 page9_data[16] = {0};
	stick_ReturnCode_t ret;
	u8 retry_cnt = 10;
	u8 exit_retry_cnt = 10;
	...
	ret = stickapi_read_page(page9_data, PAGE_INDEX9, 16);
	if (ret != 0) {
		pr_err("%s read fail\n", __func__);
		return ret;
	}

	get_count = page9_data[15] & 0xff;
	if (get_count == 0xff) {
		get_count = 0;
		page9_data[15] = 0;
		ret = stickapi_write_page(page9_data, PAGE_INDEX9, 16);
		if (ret != 0) {
			pr_err("%s write fail\n", __func__);
			return ret;
		}
	...
	if (get_count >= 29) {
		ret = stickapi_read_dc_value(&get_dc_value);
		if (get_dc_value == DC_INIT_VALUE)
			*cycle_count = get_count;
		else
			*cycle_count = DC_INIT_VALUE - get_dc_value;
	} else {
		*cycle_count = get_count;
	}
...
int st_set_cycle_count(struct auth_device * auth_dev, u32 set_cycle_count, u32 get_cycle)
{
	u8 page9_data[16] = {0};
...
static struct auth_ops st_auth_ops = {
	...
	.get_first_usage_date = st_get_first_usage_date,
	.set_first_usage_date = st_set_first_usage_date,
	.get_ui_soh = st_get_ui_soh,
	.set_ui_soh = st_set_ui_soh,
	.get_cycle_count = st_get_cycle_count,
	.set_cycle_count = st_set_cycle_count,
};
...
static int stick_platform_probe(struct platform_device *pdev)
{
	...
	if (secret_ic_str) {
		...
		if (secret_ic != 2) {
			pr_err("%s: stick secret ic didn't probed in lk\n", __func__);
			return -ENODEV;
		}
	} else {
...
#error "NO_BENCHMARK requires TWEAK_* to be defined"
	...
	for (i = 0; i < 5; i++) {
		...
		DEV_WARN(sd->dev,
				 "%s : Error while ping with stick device, retry %d times\n",
				 __func__, i);
		msleep(5);
	}
