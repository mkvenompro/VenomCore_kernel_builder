...
__maybe_unused static int nu2115_dump_reg(struct nu2115 *chip)
{
	int ret;
	int i;
	u8 val;
	...
	for (i = 0; i <= 0x36; i++) {
		ret = nu2115_read_byte(chip, i, &val);
		nu_err("%s reg[0x%02x] = 0x%02x\n", __func__, i, val);
	}

	return ret;
}

__maybe_unused static int nu2115_enable_cp_ts_detect(struct nu2115 *chip, int ts_type, bool enable)
{
	int ret;
	u8 val;

	nu_err("nu2115_enable_cp_ts_detect ts_type = %d, enable = %d\n", ts_type, enable);

	if (ts_type == TS_BAT) {
		val = enable ? NU2115_TSBAT_ENABLE : NU2115_TSBAT_DISABLE;
		val <<= NU2115_TSBAT_DIS_SHIFT;

		ret = nu2115_update_bits(chip, NU2115_REG_0E, NU2115_TSBAT_DIS_MASK, val);
	} else if (ts_type == TS_BUS) {
		val = enable ? NU2115_TSBUS_ENABLE : NU2115_TSBUS_DISABLE;
		val <<= NU2115_TSBUS_DIS_SHIFT;

		ret = nu2115_update_bits(chip, NU2115_REG_0E, NU2115_TSBUS_DIS_MASK, val);
	} else if (ts_type == TS_DIE) {
		val = enable ? NU2115_TDIE_ENABLE : NU2115_TDIE_DISABLE;
		val <<= NU2115_TDIE_DIS_SHIFT;

		ret = nu2115_update_bits(chip, NU2115_REG_0E, NU2115_TDIE_DIS_MASK, val);
	} else {
		nu_err("nu2115 charger unsupported ts type: %d\n", ts_type);
		ret = -1;
	}
...
__maybe_unused static int nu2115_enable_charge(struct nu2115 *chip, bool en)
{
	...
	/*
	 * Increase PMID2VOUT OVP threshold to increase the probability of
	 * a successful opening the cp.
	 * The threshold in 2:1 mode = ((PMID / 2) - VOUT) / VOUT
	 * The threshold in 1:1 mode = (PMID - VOUT) / VOUT
	 */
	nu2115_write_byte(chip, NU2115_REG_32, 0x70);

	// Workaround, reinit cp if TSBAT/TSBUS reset to default enable
	ret = nu2115_read_byte(chip, NU2115_REG_0E, &val);
	if (!ret) {
		if (!(val & BIT(1)) || !(val & BIT(2))) {
			nu_err("nu2115 charger TSBAT/TSBUS reset to default enable, reinit device\n");
	...
	/* Restore PMID2VOUT OVP threshold */
	nu2115_write_byte(chip, NU2115_REG_32, 0x50);
...
__maybe_unused static int nu2115_get_adc_data(struct nu2115 *chip, int channel, int *data)
{
	...
	if ((channel == NU2115_ADC_TSBUS) || (channel == NU2115_ADC_TSBAT))
		val = val * 9766/100000;
	else if (channel == NU2115_ADC_TDIE)
		val = val * 5/10;
		...
		struct device_attribute *attr, char *buf)
{
	...
	u8 addr;
	u8 val;
	u8 tmpbuf[64]; // 64bytes buffer is enough to store string
	int len;
	int idx = 0;
	int ret;
...
__maybe_unused static int nu2115_init_adc(struct nu2115 *chip)
{
	...
	nu2115_set_adc_scan(chip, NU2115_ADC_IBUS, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_VBUS, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_VOUT, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_VBAT, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_IBAT, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_TSBUS, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_TSBAT, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_TDIE, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_VAC1, true);
	nu2115_set_adc_scan(chip, NU2115_ADC_VAC2, true);
...
__maybe_unused static int nu2115_init_device(struct nu2115 *chip)
{
	...
	nu2115_enable_cp_ts_detect(chip, TS_BAT, false);
	nu2115_enable_cp_ts_detect(chip, TS_BUS, false);
	...
	/*
	 * To ensure IBUS UCP fall enabled when ibus increase slowly
	 * TODO: confirm with FAE
	 */
...
static inline enum nu2115_adc_ch to_nu2115_adc(enum cp_adc_channel channel)
{
	enum nu2115_adc_ch ch = NU2115_ADC_MAX_NUM;
...
static int nu2115_get_adc_value(struct chargerpump_dev *charger_pump, enum cp_adc_channel ch, int *value)
{
	struct nu2115 *chip = chargerpump_get_private(charger_pump);
...
static int nu2115_get_cp_workmode(struct chargerpump_dev *charger_pump, int *workmode)
{
	...
	if (!ret)
		*workmode = (val == 0x20 ? NU2115_1_1_MODE :
			(val == 0 ? NU2115_2_1_MODE : NU2115_2_1_MODE));

	dev_info(chip->dev, "%s: workmode = %d", __func__, *workmode);

	return ret;
}
#endif

static int nu2115_dump_register(struct chargerpump_dev *charger_pump)
{
	struct nu2115 *chip = chargerpump_get_private(charger_pump);
	int ret = 0;

	ret = nu2115_dump_reg(chip);
...
#endif
	.dump_cp_register = nu2115_dump_register,
};
...
static const struct of_device_id nu2115_charger_match_table[] = {
	{
...
int nu2115_driver_init(void)
{
	int rc;
	rc = i2c_add_driver(&nu2115_charger_driver);
	if (rc)
