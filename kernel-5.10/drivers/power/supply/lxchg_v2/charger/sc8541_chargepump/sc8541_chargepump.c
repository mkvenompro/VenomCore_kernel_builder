...
static const u32 sc8541_adc_accuracy_tbl[SC8541_ADC_MAX_NUM] = {
	150000,	/* IBUS */
	35000,	/* VBUS */
	35000,	/* VAC1 */
	35000,	/* VAC2 */
...
enum sc8541_notify {
	...
	SC8541_NOTIFY_VACOVP,
	SC8541_NOTIFY_TDIEFLT,
};
...
static struct intr_flag sc8541_intr_flag[] = {
	{ .reg = 0x18, .len = 7, .bit = {
	...
	{ .reg = 0x1a, .len = 8, .bit = {
				...
				{.mask = BIT(6), .name = "vac2 ovp flag", .notify = SC8541_NOTIFY_VACOVP},
				{.mask = BIT(7), .name = "vac1 ovp flag", .notify = SC8541_NOTIFY_VACOVP},
				},
	},
	{ .reg = 0x1b, .len = 8, .bit = {
				{.mask = BIT(0), .name = "wd timeout flag", .notify = SC8541_NOTIFY_OTHER},
				{.mask = BIT(1), .name = "tdie alm flag", .notify = SC8541_NOTIFY_TDIEFLT},
...
enum sc8541_fields {
	...
	SS_TIMEOUT_DIS, PMID2OUT_UVP_DIS,
				...
				enum sc8541_fields field_id, int *val)
{
	...
	if (ret < 0) {
		lx_err("sc8541 read field %d fail: %d\n", field_id, ret);
#if IS_ENABLED(CONFIG_MIEV)
				...
				int reg, uint8_t *val, int len)
{
	...
	if (ret < 0) {
		lx_err("sc8541 read %02x block failed %d\n", reg, ret);
#if IS_ENABLED(CONFIG_MIEV)
	xmdfs_notifier_call_chain(CHG_DFX_CP_I2C_ERR, NULL);
#endif
	}
...
__maybe_unused static int sc8541_detect_device(struct sc8541_chip *sc)
{
	...
	if (val != SC8541_DEVICE_ID) {
		lx_err("not find SC8541, ID = 0x%02x\n", ret);
		return -EINVAL;
	}
...
__maybe_unused static int sc8541_get_status(struct sc8541_chip *sc, uint32_t *status)
{
	...
	*status = 0;
	...
	if (ret < 0) {
		lx_err("fail to read VBUS_ERRORHI_STAT(%d)\n", ret);
		return ret;
	}
	if (val != 0)
	...
	if (ret < 0) {
		lx_err("%s fail to read VBUS_ERRORLO_STAT(%d)\n", ret);
		return ret;
	}
	if (val != 0)
...
__maybe_unused static int sc8541_init_device(struct sc8541_chip *sc)
{
	...
	sc->ignore_fault = true;
	ret = sc8541_reg_reset(sc);
	if (ret < 0) {
		lx_err("Failed to reset registers(%d)\n", ret);
	}
	mdelay(10);
	...
	if (sc->mode == SC8541_SLAVE) {
		...
		if (ret < 0) {
			lx_err("Failed to set vbus in range(%d)\n", ret);
		}
	}
	sc->ignore_fault = false;
...
__maybe_unused static int sc8541_reset_register(struct sc8541_chip *sc)
{
	int ret = 0;
	int i;
	struct {
...
static inline enum sc8541_adc_ch to_sc8541_adc(enum cp_adc_channel channel)
{
	enum sc8541_adc_ch ch = SC8541_ADC_MAX_NUM;
...
static int sc_sc8541_get_adc_value(struct chargerpump_dev *charger_pump, enum cp_adc_channel ch, int *value)
{
	struct sc8541_chip *sc = chargerpump_get_private(charger_pump);
	int ret = 0;

	ret = sc8541_get_adc_data(sc, to_sc8541_adc(ch), value);
...
static int sc_sc8541_get_cp_workmode(struct chargerpump_dev *charger_pump, int *workmode)
{
	...
	*workmode = val;
	...
	lx_info("get cp workmode = %d", val);
	return ret;
}
#endif

static int sc_sc8541_dump_register(struct chargerpump_dev *charger_pump)
{
	struct sc8541_chip *sc = chargerpump_get_private(charger_pump);
	int ret = 0;
...
static int sc_sc8541_reset_register(struct chargerpump_dev *charger_pump)
{
	struct sc8541_chip *sc = chargerpump_get_private(charger_pump);
	int ret;
	sc->ignore_fault = true;
	ret = sc8541_reset_register(sc);
	if (ret < 0) {
		lx_err("reset register failed(%d)\n", ret);
	}
	sc->ignore_fault = false;
	return ret;
}

static int sc_sc8541_set_otg_enable(struct chargerpump_dev *charger_pump, bool enable)
{
	struct sc8541_chip *sc = chargerpump_get_private(charger_pump);
	int ret = 0;

	if (enable) {
		lx_info("[REVCHG]  is sc_sc8541_set_otg_confige\n");
		sc8541_dump_reg(sc);
		ret = sc8541_field_write(sc, MODE, 0);	//workmode 2:1
		ret |= sc8541_field_write(sc, IBUS_UCP_DIS, 1);	//disable ibus_ucp
		ret |= sc8541_field_write(sc, VBUS_IN_RANGE_DIS, 1);	//disable vbus_in_range
		ret |= sc8541_field_write(sc, SS_TIMEOUT_DIS, 1);	//disable ss_timeout
		ret |= sc8541_field_write(sc, PMID2OUT_UVP_DIS, 1);	//disable pimd2out_uvp
		ret |= sc8541_enable_charge(sc, 1);	//enable cp
		mdelay(200);
		lx_info("[REVCHG] in the end : sc8541_dump_reg\n");
		sc8541_dump_reg(sc);
	} else {
		ret |= sc8541_field_write(sc, IBUS_UCP_DIS, 0);	//enable ibus_ucp
		ret |= sc8541_field_write(sc, VBUS_IN_RANGE_DIS, 0);	//enable vbus_in_range
		ret |= sc8541_field_write(sc, SS_TIMEOUT_DIS, 0);	//enable ss_timeout
		ret |= sc8541_field_write(sc, PMID2OUT_UVP_DIS, 0);	//enable pimd2out_uvp
		ret |= sc8541_enable_charge(sc, 0);	//disable cp
	}
	return ret;
}

static char* sc_sc8541_show_name(struct chargerpump_dev *charger_pump)
{
	return "sc8541";
}
...
#endif
	.dump_cp_register = sc_sc8541_dump_register,
	.reset_cp_register = sc_sc8541_reset_register,
	.set_otg_enable = sc_sc8541_set_otg_enable,
	.get_name = sc_sc8541_show_name,
};
...
*/
static void sc8541_notify_fault_status(struct sc8541_chip *sc, int notify)
{
#if IS_ENABLED(CONFIG_MIEV)
	...
	case SC8541_NOTIFY_VBATOVP:
		sc8541_get_adc_data(sc, SC8541_ADC_VBUS, &info.data[0]);
		mdelay(100);
		sc8541_get_adc_data(sc, SC8541_ADC_VBUS, &info.data[1]);
	...
	case SC8541_NOTIFY_VACOVP:
		xmdfs_notifier_call_chain(CHG_DFX_CP_VAC_OVP, NULL);
		break;

	case SC8541_NOTIFY_TDIEFLT:
...
static void sc8541_check_fault_status(struct sc8541_chip *sc)
{
	...
	for (i = 0; i < ARRAY_SIZE(sc8541_intr_flag); i++) {
		ret = sc8541_read_block(sc, sc8541_intr_flag[i].reg, &flag, 1);
		for (j=0; j <  sc8541_intr_flag[i].len; j++) {
			if (flag & sc8541_intr_flag[i].bit[j].mask && !sc->ignore_fault) {
				sc8541_notify_fault_status(sc, sc8541_intr_flag[i].bit[j].notify);
				lx_info("trigger :%s\n", sc8541_intr_flag[i].bit[j].name);
				sc8541_dump_reg(sc);
			}
		}
	}
...
static int sc8541_register_interrupt(struct sc8541_chip *sc)
{
	...
	if (gpio_is_valid(sc->irq_gpio)) {
		...
		if (ret) {
			lx_err("failed to request sc8541_irq\n");
			return -EINVAL;
		}
		sc->irq = gpio_to_irq(sc->irq_gpio);
		if (sc->irq < 0) {
			lx_err("failed to gpio_to_irq\n");
			return -EINVAL;
		}
	} else {
		lx_err("irq gpio not provided\n");
		return -EINVAL;
	}
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		ret = sc8541_get_adc_data(sc, SC8541_ADC_VBUS, &result);
		if (!ret)
			sc->vbus_volt = result;
		val->intval = sc->vbus_volt;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		ret = sc8541_get_adc_data(sc, SC8541_ADC_IBUS, &result);
		if (!ret)
			sc->ibus_curr = result;
		val->intval = sc->ibus_curr;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		ret = sc8541_get_adc_data(sc, SC8541_ADC_VBAT, &result);
		if (!ret)
			sc->vbat_volt = result;
		val->intval = sc->vbat_volt;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		ret = sc8541_get_adc_data(sc, SC8541_ADC_IBAT, &result);
		if (!ret)
			sc->ibat_curr = result;
		val->intval = sc->ibat_curr;
		break;
	case POWER_SUPPLY_PROP_TEMP:
		ret = sc8541_get_adc_data(sc, SC8541_ADC_TDIE, &result);
		if (!ret)
			sc->die_temp = result;
			...
			const struct i2c_device_id *id)
{
	...
	if (IS_ERR(sc->regmap)) {
		lx_err("Failed to initialize regmap\n");
	...
	if (ret) {
		lx_err("Fail to set work mode!\n");
		goto err_set_mode;
	}

	ret = sc8541_parse_dt(sc, &client->dev);
	if (ret < 0) {
		lx_err("parse dt failed(%d)\n", ret);
		goto err_parse_dt;
	}

	ret = sc8541_init_device(sc);
	if (ret < 0) {
		lx_err("init device failed(%d)\n", ret);
	...
	lx_err("sc8541[%s] probe successfully!\n",
...
int sc8541_chargepump_init(void)
{
	int rc;
	rc = i2c_add_driver(&sc8541_charger_driver);
	if (rc)
...
MODULE_AUTHOR("South Chip <Aiden-yu@LIXUN.com>");
