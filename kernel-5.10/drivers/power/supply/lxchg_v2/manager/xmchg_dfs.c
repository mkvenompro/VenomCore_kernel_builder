...
module_param_named(boot_complete, boot_complete, bool, S_IRUGO | S_IWUSR);
...
static void xm_chg_dfx_handle_event_report(struct xm_chg_dfs *chg_dfs, struct xm_chg_dfs_evt *dfs_evt)
{
	...
	if (boot_complete && dfs_evt->event_trig(chg_dfs, dfs_evt) &&
		...
		lx_info("dfs_type = %d, dfs_id = %d, report_cnt = %d, report_time = %llds, para_str = %s\n",
...
void handle_evt_report_check_work(struct work_struct *work)
{
	...
	if (!manager->auth_dev)
		chg_dfs->battery.batt_auth = false;
	else
		chg_dfs->battery.batt_auth = true;

	mutex_lock(&chg_dfs->report_check_lock);
	manager->boot_complete = boot_complete;

	lx_info("boot_complete: %d, cycle_cnt: %d, chg_status: %d , soc: %d, rsoc: %d, vbat: %d, tbat: %d, chg_type: %d, vbus: %d, tdie: %d, batt_auth: %d\n",
		boot_complete, chg_dfs->battery.cycle_cnt, chg_dfs->battery.chg_status, chg_dfs->battery.soc, chg_dfs->battery.rsoc, chg_dfs->battery.vbat,
		chg_dfs->battery.tbat, manager->real_type,chg_dfs->buck.vbus,chg_dfs->pump.tdie,chg_dfs->battery.batt_auth);
