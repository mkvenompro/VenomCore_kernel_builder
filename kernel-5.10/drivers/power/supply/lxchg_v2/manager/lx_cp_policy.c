...
static void pdm_multi_mode_switch(struct chargerpump_policy *policy)
{
   ...
   if (policy == NULL)
	...
	lx_info("ibat_request = %d, 2_1 mode = %d, 1_1 mode = %d, soc = %d\n",
...
static bool update_smart_chg_cp_workmode(struct chargerpump_policy *policy)
{
	...
	lx_info("master cp mode = %d, slave cp mode = %d\n", policy->m_cp_mode, policy->s_cp_mode);

	if (!policy->manager) {
		lx_info("call monitor_smart_chg\n");
		if(policy->manager->pps_fast_mode){
			if (policy->switch2_1_enable && (!((policy->m_cp_mode == 0) && (policy->s_cp_mode == 0)))) {
				...
				lx_info("low_fast working, set switch2_1_enable\n");
			}
			lx_info("low_fast working, cp_mode not switch\n");
			return false;
		}
		else
			return false;
	...
	if (policy->switch2_1_enable && (!((policy->m_cp_mode == 0) && (!policy->cfg.s_cp_enable || policy->s_cp_mode == 0)))) {
		...
		chargerpump_set_cp_workmode(policy->master_cp, 0);
		chargerpump_set_cp_workmode(policy->slave_cp, 0);
		lx_info("N19A:set switch2_1_enable\n");
		return true;
		...
		lx_info("N19A:set switch1_1_enable\n");
		return true;
		...
		msleep(1000);
		chargerpump_set_cp_workmode(policy->master_cp, 0);
		lx_info("N19A:set switch2_1_single_enable\n");
		return true;
		...
		msleep(1000);
		chargerpump_set_cp_workmode(policy->master_cp, 1);
		lx_info("N19A:set switch1_1_single_enable\n");
		return true;
	}
	return false;
}
...
static int chargerpump_policy_update_chg_info(struct chargerpump_policy *policy)
{
	...
	if (policy->adapter == NULL) {
		...
		if (policy->adapter == NULL) {
			lx_err("can not find pd adapter\n");
			return -EINVAL;
		}
	}
	...
	if (policy->charger == NULL) {
		...
		if (policy->charger == NULL) {
			lx_err("can not find charger dev\n");
			return -EINVAL;
		}
	}
	...
	if (policy->master_cp == NULL) {
		...
		if (policy->master_cp == NULL) {
			lx_err("can not find master_cp dev\n");
			return -EINVAL;
		}
	}
	...
	if (cfg->s_cp_enable) {
		if (policy->slave_cp == NULL) {
			...
			if (policy->slave_cp == NULL) {
				lx_err("can not find slave_cp dev\n");
				return -EINVAL;
			}
		}
	}
	...
	if (policy->fuel_gauge == NULL) {
		...
		if (!policy->fuel_gauge) {
			lx_err("can not find fuel_gauge device\n");
			return -EINVAL;
		}
	}
#endif
	...
	if (ret < 0) {
		lx_err("failed to get ibat\n");
	}
	info->ibat = pval.intval / 1000;

	ret = power_supply_get_property(policy->batt_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);
	if (ret < 0) {
		lx_err("failed to get vbat\n");
	}
	info->vbat = pval.intval / 1000;

	ret = power_supply_get_property(policy->batt_psy, POWER_SUPPLY_PROP_TEMP, &pval);
	if (ret < 0) {
		lx_err("failed to get tbat\n");
	}
	info->tbat = pval.intval;

	if(cfg->s_cp_enable){
		ret = chargerpump_get_adc_value(policy->master_cp, CP_ADC_IBUS, &(info->m_ibus));
		if (ret < 0) {
			lx_err("failed to get m_ibus\n");
		}
		ret = chargerpump_get_adc_value(policy->slave_cp, CP_ADC_IBUS, &(info->s_ibus));
		if (ret < 0) {
			lx_err("failed to get s_ibus\n");
		}
		info->ibus = info->m_ibus + info->s_ibus;
	} else {
		ret = chargerpump_get_adc_value(policy->master_cp, CP_ADC_IBUS, &(info->ibus));
		if (ret < 0) {
			lx_err("failed to get ibus\n");
		}
	}

	ret = charger_get_adc(policy->charger, CHG_ADC_VBUS, &(info->vbus));
	if (ret < 0) {
		lx_err("failed to get vbus\n");
	}

	ret = get_effective_result(policy->manager->fv_votable);
	if (ret < 0) {
		lx_err("failed to get total_fcc\n");
	}
	info->ffc_cv = ret;

	ret = get_effective_result(policy->manager->total_fcc_votable);
	if (ret < 0) {
		lx_err("failed to get total_fcc\n");
	}
	info->total_fcc = ret;

	ret = get_effective_result(policy->manager->iterm_votable);
	if (ret < 0) {
		lx_err("failed to get iterm_votable\n");
	}
	info->iterm = ret;

	ret = get_effective_result(policy->manager->input_limit_votable);
	if (ret < 0) {
		lx_err("failed to get input_limit_votable\n");
	}
	info->input_suspend = !ret;
	...
	//chargerpump_dump_register(policy->master_cp);
...
static int chargerpump_policy_state_init(struct chargerpump_policy *policy)
{
	...
	if (info->soc >= CHARGERPUMP_POLICY_HIGH_SOC && !info->soft_reset_state) {
		lx_err("soc too high\n");
		policy->next_time = 50;
		policy->recover = false;
		policy->sm = PM_STATE_CHARGERPUMP_EXIT;
	} else if (info->total_fcc < cfg->exit_cc || info->input_suspend) {
		lx_err("total fcc too low or input_suspend or night_charging is true\n");
	...
	if (info->soft_reset_state)
			info->soft_reset_state = false;
...
{
	rerun_election(policy->manager->total_fcc_votable);
...
static int chargerpump_policy_state_enable_chargerpump(struct chargerpump_policy *policy)
{
	...
	if (cnt++ > CHARGERPUMP_POLICY_ENABLE_RETRY_CNT) {
		lx_info("enable chargerpump failed\n");
		cnt = 0;
		chargerpump_set_enable(policy->master_cp, false);
		if(policy->cfg.s_cp_enable) {
			chargerpump_set_enable(policy->slave_cp, false);
		...
		policy->next_time = 50;
#if IS_ENABLED(CONFIG_MIEV)
		xmdfs_notifier_call_chain(CHG_DFX_CP_ENABLE_FAIL, NULL);
#endif
		goto out;
	}
...
static int __chargerpump_cc_cv_algo(struct chargerpump_policy *policy)
{
	...
	uint32_t final_cv;
	static int blo_200, blo_150;
	const char *effect_voter = get_effective_client(policy->manager->total_fcc_votable);
	...
	if ((info->vbat > (final_cv - cfg->cv_offset)) && (info->ibat < cfg->exit_cc)) {
		lx_info("cp chg done(%d)!, vbat = %d(%d), ibat = %d(%d)\n", iterm_cnt, info->vbat,
				(final_cv - cfg->cv_offset), info->ibat, cfg->exit_cc);
	...
	vbat_lmt = min((final_cv - 10), cfg->max_vbat);
	...
	if (info->total_fcc < cfg->exit_cc) {
		if (!IS_ERR_OR_NULL(effect_voter) &&
			(!strcmp(effect_voter, CHG_CYCLE_VOTER) || !strcmp(effect_voter, SOFT_ITERM_VOTER)))
			return PM_ALGO_RET_CHARGERPUMP_DONE;
		else
	...
	if (abs(info->vbat - vbat_lmt) < 100)
		policy->fast_step = false;
	...
	if (abs(info->ibus - ibus_lmt) < 400)
		policy->fast_step = false;

	// ibat loop
	if (info->tbat > 450 && info->total_fcc > 2500) {
		if (info->ibat > ibat_lmt) {
			step_ibat = -cfg->step_volt;
		} else if (info->ibat < ibat_lmt - 300) {
			step_ibat = cfg->step_volt;
		}

		if ((info->ibat >= ibat_lmt - 300) && (info->ibat < ibat_lmt - 100)) {
			if (info->ibat < ibat_lmt - 200) {
				blo_200++;
				blo_150++;
			} else if (info->ibat < ibat_lmt - 150) {
				blo_150++;
				blo_200 = 0;
			}
		...
		if (info->ibat < ibat_lmt - 100) {
			...
			if (info->ibat - ibat_lmt > 300)
				step_ibat = -2 * cfg->step_volt;
			else
				step_ibat = -cfg->step_volt;
		}
	}

	if (abs(info->ibat - ibat_lmt) < 800)
		policy->fast_step = false;

	if (info->ibat == info->last_ibat) {
		policy->fast_step = false;
		step_ibat = 0;
		lx_info("fg has not been updated \n");
	}
	...
	if (policy->request_volt >= cfg->max_request_volt)
		policy->request_volt = cfg->max_request_volt;

	policy->request_curr = ibus_lmt;

	lx_info("fast_step=%d, rqst_volt=%d(%d), rqst_curr= %d(%d), ibat_lmt=%d(%d), vbat_lmt=%d(%d)\n",
				policy->fast_step, policy->request_volt,info->vbus, ibus_lmt,info->ibus,
				ibat_lmt,info->ibat, vbat_lmt,info->vbat);

	lx_info("cp algo: %d %d %d, %d, %d %d\n", step_vbat, step_ibat, step_ibus, step, blo_200, blo_150);
...
static int chargerpump_policy_state_chargerpump_cc_cv(struct chargerpump_policy *policy)
{
	...
	if (ret == PM_ALGO_RET_SOFT_RESET) {
		...
		policy->sm = PM_STATE_CHARGERPUMP_EXIT;
		policy->next_time = 50;
		lx_info("soft reset, exit state machine.\n");
		return 0;
	}
	...
	if (ret == PM_ALGO_RET_CHARGERPUMP_DONE) {
		...
		policy->recover = false;
		policy->sm = PM_STATE_CHARGERPUMP_EXIT;
		lx_info("cp charge done, exit state machine.\n");
	}
	...
	if (ret == PM_ALGO_RET_CP_LMT_CURR) {
		...
		policy->sm = PM_STATE_CHARGERPUMP_EXIT;
		policy->next_time = 50;
		lx_info("total fcc less than min fast chg limit, stop state machine.\n");
		return 0;
	}
	...
	if (ret == PM_ALGO_RET_INPUT_SUSPEND) {
		...
		policy->sm = PM_STATE_CHARGERPUMP_EXIT;
		policy->next_time = 50;
		lx_info("input_suspend is true, stop state machine.\n");
		return 0;
	}
...
static int dv_policy_state_chargerpump_exit(struct chargerpump_policy *policy)
{
	...
	lx_info("m_cp_chging=%d, s_cp_chging=%d\n", info->m_cp_chging, info->s_cp_chging);

	// Whether master cp or slave cp exit, disable master cp first and then slave cp
	if (info->m_cp_chging || (cfg->s_cp_enable && info->s_cp_chging)) {
		chargerpump_set_enable(policy->master_cp, false);
		chargerpump_policy_check_cp_enable(policy);
	...
	chargerpump_policy_set_state(policy, POLICY_STOP);
	rerun_election(policy->manager->total_fcc_votable);
	rerun_election(policy->manager->icharge_votable);
	msleep(500);
	...
	while (true) {
		...
		if (info->total_fcc > 0) {
			...
			if (need_switch) {
				policy->sm = PM_STATE_INIT;
				policy->initial_vbus_flag = false;
				lx_info("need switch cp mode\n");
			}
		} else
			policy->initial_vbus_flag = false;
#endif
		lx_info("state phase [%s]: bat_info=[%d, %d, %d],bus_info=[%d,%d,%d,%d,%d], total_fcc = %d\n",
			sm_str[policy->sm], info->ibat, info->vbat, info->tbat, info->vbus,
			info->ibus, info->m_ibus, info->s_ibus, info->tadapter, info->total_fcc);
										...
										struct adapter_cap *cap)
{
	...
	for (i = 0; i < cap->cnt; i++) {
		lx_info("max volt : %d, min volt : %d, max curr : %d, min curr : %d\n",
				cap->volt_max[i], cap->volt_min[i], cap->curr_max[i], cap->curr_min[i]);
		//ignore fix gear
		if (cap->volt_min[i] == cap->volt_max[i])
			continue;

		if (cap->volt_min[i] >= cfg->min_request_volt &&
			cap->curr_max[i] >= cfg->min_request_curr) {
			if (cap->volt_max[i] >= cfg->max_request_volt)
				max_pwr = cfg->max_request_volt * cap->curr_max[i] / 1000000;
			else
	...
	lx_info("the final choice: max volt : %d, min volt : %d, max curr : %d, min curr : %d\n",
...
static struct adapter_dev *chargerpump_policy_check_adapter(struct chargerpump_policy *policy)
{
	...
	for (i = 0; i < ARRAY_SIZE(policy->adapter_name); i++) {
		...
		adapter = adapter_find_dev_by_name("lx_pd_adapter");
		if (adapter == NULL)
			continue;

		ret = adapter_handshake(adapter);
		if (ret < 0) {
			lx_err("%s adapter handshake failed\n", policy->adapter_name[i]);
			goto out;
		}

		ret = adapter_get_cap(adapter, &policy->cap);
		if (ret < 0) {
			lx_err("%s adapter get cap failed\n", policy->adapter_name[i]);
			goto out;
		}
...
static int dv_policy_state_check_dev(struct chargerpump_policy *policy)
{
	...
	//reset registers
	chargerpump_reset_register(policy->master_cp);
...
int chargerpump_policy_start(struct chargerpump_policy *policy)
{
	...
	chargerpump_policy_wake_thread(policy);
	policy->info.soft_reset_state = false;
	return 0;
}
...
static int chargepump_policy_parse_dt(struct charger_manager *manager)
{
	int ret = 0;
	struct device_node *np = of_find_node_by_name(manager->dev->of_node, "lx_cp_policy");
	struct chargerpump_policy *policy = manager->cp_policy;

	ret |= of_property_read_u32(np, "lx_cp_policy,cv_offset", &policy->cfg.cv_offset);
	ret |= of_property_read_u32(np, "lx_cp_policy,exit_cc", &policy->cfg.exit_cc);
	ret |= of_property_read_u32(np, "lx_cp_policy,step_volt", &policy->cfg.step_volt);
	ret |= of_property_read_u32(np, "lx_cp_policy,warm_temp", &policy->cfg.warm_temp);
	ret |= of_property_read_u32(np, "lx_cp_policy,max_request_volt", &policy->cfg.max_request_volt);
	ret |= of_property_read_u32(np, "lx_cp_policy,min_request_volt", &policy->cfg.min_request_volt);
	ret |= of_property_read_u32(np, "lx_cp_policy,max_request_curr", &policy->cfg.max_request_curr);
	ret |= of_property_read_u32(np, "lx_cp_policy,min_request_curr", &policy->cfg.min_request_curr);
	ret |= of_property_read_u32(np, "lx_cp_policy,max_ibat", &policy->cfg.max_ibat);
	ret |= of_property_read_u32(np, "lx_cp_policy,max_vbat", &policy->cfg.max_vbat);
	ret |= of_property_read_u32(np, "lx_cp_policy,min_vbat", &policy->cfg.min_vbat);
	if (ret) {
		lx_err("failed to parse policy configurations from device tree\n");
		return ret;
	}

	policy->cfg.s_cp_enable = of_property_read_bool(np, "lx_cp_policy,s_cp_enable");

	lx_info("policy cfg - cv_offset:%u, exit_cc:%u, step_volt:%u, warm_temp:%u\n",
			policy->cfg.cv_offset, policy->cfg.exit_cc, policy->cfg.step_volt, policy->cfg.warm_temp);
	lx_info("policy cfg - max_request_volt:%u, min_request_volt:%u, max_request_curr:%u, min_request_curr:%u\n",
			policy->cfg.max_request_volt, policy->cfg.min_request_volt,
			policy->cfg.max_request_curr, policy->cfg.min_request_curr);
	lx_info("policy cfg - max_ibat:%u, max_vbat:%u, min_vbat:%u, s_cp_enable:%d\n",
			policy->cfg.max_ibat, policy->cfg.max_vbat, policy->cfg.min_vbat, policy->cfg.s_cp_enable);
...
{
	struct chargerpump_policy *pump_policy;

	lx_info("start\n");
	pump_policy = devm_kzalloc(manager->dev, sizeof(*pump_policy), GFP_KERNEL);
	if (!pump_policy)
		return -ENOMEM;

	manager->cp_policy = pump_policy;
	pump_policy->manager = manager;

	init_waitqueue_head(&pump_policy->wait_queue);

	memcpy(&pump_policy->cfg, &default_cfg, sizeof(default_cfg));
	chargepump_policy_parse_dt(manager);

	pump_policy->thread = kthread_run(chargerpump_policy_thread_fn, pump_policy,
							"chargerpump_policy_thread");

	//policy->adapter_name[0] = "vfcp adapter";
	//policy->adapter_name[1] = "ufcs_port2";
	pump_policy->adapter_name[0] = "pd_adapter";

	mutex_init(&pump_policy->state_lock);
	mutex_init(&pump_policy->running_lock);
	mutex_init(&pump_policy->access_lock);
