		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (adapter_dev == NULL || adapter_dev->ops == NULL ||
		adapter_dev->ops->set_pd_verify_process == NULL)
		return -EINVAL;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (adapter_dev == NULL || adapter_dev->ops == NULL ||
		adapter_dev->ops->get_power_role == NULL)
		return -EINVAL;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (adapter_dev == NULL || adapter_dev->ops == NULL ||
		adapter_dev->ops->get_current_state == NULL)
		return -EINVAL;
...
int xm_adapter_class_init(void)
{
