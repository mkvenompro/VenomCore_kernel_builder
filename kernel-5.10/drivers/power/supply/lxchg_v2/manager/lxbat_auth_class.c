...
{
	if (!auth_device || !auth_device->ops)
		return "UNKNOWN";
	if (auth_device->ops->get_name == NULL)
		return "UNKNOWN";
	return auth_device->ops->get_name(auth_device);
}
EXPORT_SYMBOL(auth_get_name);
