...
{
	get_lcd_name = flag;
	pr_info("[GTP][FTS_TS]%s get_lcd_name:%s \n", __func__, get_lcd_name);
}
...
{
	int panel_id = -1;
	if(strstr(get_lcd_name,"panel_name=dsi_p7_36_0d_0a_dsc_vdo")) {
		panel_id = 0;   //TM
	} else if (strstr(get_lcd_name, "panel_name=dsi_p7_41_02_0b_dsc_vdo")) {
		panel_id = 1;   //VIOX
	} else if (strstr(get_lcd_name, "panel_name=dsi_p7_35_0f_0c_dsc_vdo")){
		panel_id = 2;   //BOE
	} else {
