...
ssize_t tp_selftest_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	char *ini_file_name_tm = "ft3683g_p7_tm.ini";
	char *ini_file_name_boe = "ft3683g_p7_boe.ini";
    char *ini_file_name_tm_cg = "ft3683g_p7_tm_cg.ini";
	char *ini_file_name_boe_cg = "ft3683g_p7_boe_cg.ini";
	...
	if(goodix_focal_panel_name() == 0){
        if (fw_db_num == 2)
            ret = fts_test_get_testparams(ini_file_name_tm);
        else if (fw_db_num == 1)
            ret = fts_test_get_testparams(ini_file_name_tm_cg);
    } else if (goodix_focal_panel_name() == 2){
        if (fw_db_num == 3)
            ret = fts_test_get_testparams(ini_file_name_boe);
        else if (fw_db_num == 4)
            ret = fts_test_get_testparams(ini_file_name_boe_cg);
    }
