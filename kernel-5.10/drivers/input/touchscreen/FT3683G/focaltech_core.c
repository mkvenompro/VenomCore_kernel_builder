...
#if IS_ENABLED(CONFIG_MIEV)
                xiaomi_touch_mievent_report_str(TOUCH_EVENT_TRANSFER_ERR, 0, "TpTransferErr", "focal");
#endif
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
	...
	if (enable) {
		if (ts_data->power_disabled) {
			...
			if (ts_data->pdata->iovdd_gpio > 0) {
				gpio_direction_output(ts_data->pdata->iovdd_gpio, 1);
				usleep_range(15000, 15100);
				FTS_INFO("enable iovdd gpio");
			}
...
#if IS_ENABLED(CONFIG_MIEV)
            xiaomi_touch_mievent_report_int(TOUCH_EVENT_PARAM_ERR, 0, "TpParamParseFail", "focal", ret);
#endif
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (chosen) {
		...
		if (tmp_size > 0) {
			sscanf(tmp_buf, "%02hhx", &(pdata->panel_soft_id));
			FTS_INFO("[%s]: lcm_soft_id = %s, size = %d\n", __func__, tmp_buf, tmp_size);
		} else {
			FTS_INFO("[%s]:get lcm_soft_id failed\n", __func__);
		}
	} else {
		FTS_INFO("[%s]:find chosen failed\n", __func__);
	}

	if (strstr(tmp_buf,"34")) {
		fw_db_num = 1;
		FTS_INFO("TM new CG 34");
	} else if (strstr(tmp_buf,"16")) {
		fw_db_num = 2;
		FTS_INFO("TM old CG 16");
	} else if (strstr(tmp_buf,"08")) {
		fw_db_num = 3;
		FTS_INFO("BOE old CG 08");
	} else if (strstr(tmp_buf,"32")) {
		fw_db_num = 4;
		FTS_INFO("BOE new CG 32");
	}
	...
	ret = of_get_named_gpio(np, "goodix,iovdd-gpio", 0);
	if (ret < 0) {
		FTS_ERROR("can't find iovdd-gpio, use other power supply");
		pdata->iovdd_gpio = 0;
	} else {
		FTS_INFO("get iovdd-gpio[%d] from dt", ret);
		pdata->iovdd_gpio = ret;
	}
...
static void fts_power_supply_work(struct work_struct *work)
{
	...
	if (charger_status != ts_data->charger_status || ts_data->charger_status <0) {
		...
		if(charger_status) {
			...
			ret = fts_write_reg(FTS_REG_EARPHONE_MODE_EN, true);
			if (ret < 0) {
				FTS_ERROR("failed to open ear mode");
			} else {
				FTS_INFO("success to open ear mode");
			}
		}else {
			...
			ret = fts_write_reg(FTS_REG_EARPHONE_MODE_EN, false);
			if (ret < 0) {
				FTS_ERROR("failed to close ear mode");
			} else {
				FTS_INFO("success to close ear mode");
			}
		}
	}
...
static int lct_set_input_mode(int value)
{
	int ret = -1;
	if (value != 0 && value != 1) {
		FTS_ERROR("invaild input mode cmd: %d", value);
		return -EINVAL;
        }
	ret = fts_write_reg(FTS_REG_INPUT_METHOD, value);
	if (ret < 0)
		FTS_ERROR("failed send input mode cmd, value = %d", value);
	else
		FTS_INFO("Touch_Input_Mode value: %d", value);
	return ret;
}
...
int fts_set_cur_value(int mode, int value)
{
	...
	if (mode == Touch_Is_In_Input_Method) {
		if (fts_value >= 0 && !fts_data->suspended)
			lct_set_input_mode(fts_value);
	}
	...
	} else if (mode == Touch_Game_Mode && value == 0) {
		FTS_INFO("Mode:Game_Mode  Game_Mode_status = %d",value);
		/*Enable instantaneous sampling*/
...
static int fts_set_mode_long_value(int mode, int len, int *buf)
{
	...
	FTS_INFO("enter set_mode_long : %s, mode: %d, len: %d\n",__func__, mode, len);
	...
	for (i = 0; i < len; i = i + 8) {
		if(i == 48) { //defult
			if(xiaomi_touch_interfaces.long_mode_value[i + 4] == 50) {  //defult
				fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, 0x01);
				FTS_INFO("fts_set_mode_long_value is defult");
				edge_level = 50;
			} else if(xiaomi_touch_interfaces.long_mode_value[i + 4] == 60) { //strong 
				fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, 0x02);
				FTS_INFO("fts_set_mode_long_value is strong");
				edge_level = 60;
			} else if(xiaomi_touch_interfaces.long_mode_value[i + 4] == 30) {  //weak
				fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, 0x03);
				FTS_INFO("fts_set_mode_long_value is weak");
				edge_level = 30;
			} else {  //Customize
				fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, 0x04);
				customize = xiaomi_touch_interfaces.long_mode_value[i + 4];
				edge_level = customize;
				fts_write_reg(FTS_NOMAL_CUST_FILTER_LEVEL, customize);
				FTS_INFO("fts_set_mode_long_value is Customize = %d", customize);
			}
		}
	}
	mutex_unlock(&fts_data->report_mutex);
...
int get_battery_temperature(void) {
    ...
    if (ret) {
        printk(KERN_ERR "Failed to get battery temperature: %d\n", ret);
        power_supply_put(psy);
        return ret;
    }

    FTS_INFO("Battery temperature: %d °C", val.intval / 10);
    power_supply_put(psy);
...
static int fts_ts_resume(struct device *dev)
{
    ...
    ret = fts_write_reg(FTS_TEMP_REG, temp);
    if (ret < 0)
        FTS_ERROR("write board temperature fail,ret: %d",ret);

	if (edge_level == 50) {
		fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, 0x01);
		FTS_INFO("fts_ts_resume is defult");
	} else if (edge_level == 60) {
		fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, 0x02);
		FTS_INFO("fts_ts_resume is strong");
	} else if (edge_level == 30) {
		fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, 0x03);
		FTS_INFO("fts_ts_resume is weak");
	} else {
		fts_write_reg(FTS_NOMAL_EDGE_FILTER_LEVEL, 0x04);
		fts_write_reg(FTS_NOMAL_CUST_FILTER_LEVEL, edge_level);
		FTS_INFO("fts_ts_resume is edge_level = %d", edge_level);
	}
...
static int __init fts_ts_init(void)
{
	...
	if (goodix_focal_panel_name() == 0) {
		FTS_INFO("TP is focal 3683G TM");
		focal_fw_name = 1;
	} else if (goodix_focal_panel_name() == 2) {
		FTS_INFO("TP is focal 3683G BOE");
		focal_fw_name = 2;
	} else {
