...
#define FTS_FW_NAME_PREX_WITH_REQUEST_TM               "focaltech_ts_fw_tm"
#define FTS_FW_NAME_PREX_WITH_REQUEST_BOE          "focaltech_ts_fw_boe"
#define FTS_FW_NAME_PREX_WITH_REQUEST_TM_CG               "focaltech_ts_fw_tm_cg"
#define FTS_FW_NAME_PREX_WITH_REQUEST_BOE_CG          "focaltech_ts_fw_boe_cg"
#define TM_VESION_INFO 0x27
#define BOE_VESION_INFO 0x23
    ...
    if (goodix_focal_panel_name() == 0) {
        if (fw_db_num == 2)
            snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin",
                FTS_FW_NAME_PREX_WITH_REQUEST_TM,
                upg->module_info->vendor_name);
        else if (fw_db_num == 1)
            snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin",
                FTS_FW_NAME_PREX_WITH_REQUEST_TM_CG,
                upg->module_info->vendor_name);
    } else if (goodix_focal_panel_name() == 2) {
        if (fw_db_num == 3)
            snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin",
                FTS_FW_NAME_PREX_WITH_REQUEST_BOE,
                upg->module_info->vendor_name);
        else if (fw_db_num == 4)
            snprintf(fwname, FILE_NAME_LENGTH, "%s%s.bin",
                FTS_FW_NAME_PREX_WITH_REQUEST_BOE_CG,
                upg->module_info->vendor_name);
    }
    ret = request_firmware(&fw, fwname, upg->ts_data->dev);
    if (ret == 0) {
