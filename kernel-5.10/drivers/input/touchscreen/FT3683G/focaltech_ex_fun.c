...
extern int fw_db_num;
int fts_factory_test(void)
{
    int ret = 0;
    char fwname_tm[FILE_NAME_LENGTH] = { "ft3683g_p7_tm.ini" };
    char fwname_boe[FILE_NAME_LENGTH] = { "ft3683g_p7_boe.ini" };
    char fwname_tm_cg[FILE_NAME_LENGTH] = { "ft3683g_p7_tm_cg.ini" };
    char fwname_boe_cg[FILE_NAME_LENGTH] = { "ft3683g_p7_boe_cg.ini" };
    struct fts_ts_data *ts_data = fts_data;
    ...
    if (ret < 0) {
        ...
        if (focal_ini_name == 1) {
            ret = fts_test_entry(fwname_tm_cg);
            test_result = factory_result;
            FTS_INFO("ft3683g tm_cg openshort test pass test_result = %d", test_result);
        } else if (focal_ini_name == 2) {
            ret = fts_test_entry(fwname_tm);
            test_result = factory_result;
            FTS_INFO("ft3683g tm openshort test pass test_result = %d", test_result);
        } else if (focal_ini_name == 3) {
            ret = fts_test_entry(fwname_boe);
            test_result = factory_result;
            FTS_INFO("ft3683g boe openshort test pass test_result = %d", test_result);
        } else if (focal_ini_name == 4) {
            ret = fts_test_entry(fwname_boe_cg);
            test_result = factory_result;
            FTS_INFO("ft3683g boe_cg openshort test pass test_result = %d", test_result);
        }
...
static const struct proc_ops ctp_openshort_test_fops = {
...
extern int fts_set_cur_value(int mode, int value);
static int ctp_ato_aod_show(struct seq_file *m, void *v)
{
	return 0;
}
static int tp_fod_press_open(struct inode *inode, struct file *file)
{
	return single_open(file, ctp_ato_aod_show, NULL);
}

ssize_t tp_fod_press_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	char tmp[6];
	int ret;
	int tp_selftest_result = SELFTEST_INVALID;

    FTS_INFO("tp_fod_press_write enter");
	...
	if (!strncmp(tmp, "close", 5)) {
		tp_selftest_result = fts_set_cur_value(10,0);
        FTS_INFO("focal close fod");
    } else if (!strncmp(tmp, "open", 4)){
		tp_selftest_result = fts_set_cur_value(10,1);
        FTS_INFO("focal open fod");
    }
...
static const struct proc_ops tp_fodpress_fops = {
	.proc_open = tp_fod_press_open,
	.proc_write = tp_fod_press_write,
    .proc_read = seq_read,
    .proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
int fts_create_proc(struct fts_ts_data *ts_data)
{
    ...
    struct proc_dir_entry *ctp_openshort_test = NULL;

    //add proc/touchscreen/ctp_openshort_test
    proc_touchscreen_dir = proc_mkdir("touchscreen" , NULL);
    if (proc_touchscreen_dir == NULL)  {
        FTS_INFO("create proc/touchscreen dir file failed!");
    }else{
        ctp_openshort_test = proc_create("ctp_openshort_test", 0666, proc_touchscreen_dir, &ctp_openshort_test_fops);
        if(ctp_openshort_test ==NULL){
            FTS_INFO("create ctp_openshort_test failed!");
            remove_proc_entry("touchscreen", NULL);
        }else{
            FTS_INFO("create ctp_openshort_test success!");
        }
    }

	proc->tp_fod_press_proc = proc_create("tp_fod_press", 0664, NULL, &tp_fodpress_fops);
	if (proc->tp_fod_press_proc == NULL) {
		FTS_ERROR("tp_fod_press proc create failed");
		return -ENOMEM;
	}
...
void fts_remove_proc(struct fts_ts_data *ts_data)
{
	...
	if (proc->proc_entry)
		proc_remove(proc->proc_entry);
    if (proc->tp_fod_press_proc)
		proc_remove(proc->tp_fod_press_proc);
	proc->proc_entry = NULL;
    proc->tp_fod_press_proc = NULL;
}
