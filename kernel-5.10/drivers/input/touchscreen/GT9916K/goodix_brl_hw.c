...
int brl_gesture(struct goodix_ts_core *cd, int gesture_type)
{
	...
	ts_info("BRL cmd 0 is 0x%2x", cmd.data[0]);
	ts_info("BRL cmd 1 is 0x%2x", cmd.data[1]);
...
static int brl_dev_confirm(struct goodix_ts_core *cd)
{
	...
	if (retry < 0) {
		...
		xiaomi_touch_mievent_report_int(TOUCH_EVENT_TRANSFER_ERR, 0, "TpTransferErr", "goodix", ERROR_REGULATOR_INIT);
#endif
		ret = -EINVAL;
