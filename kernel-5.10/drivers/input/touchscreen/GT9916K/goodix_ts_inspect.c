	...
	int freq_cnt;
 	...
 	if (ts_core->lockdown_info[1] == 0x36){
		if(fw_db_num == 1)
			...
			sprintf(limit_file, "%s_test_limits_%d_TM_cg.csv", GOODIX_TEST_FILE_NAME,
				ts_core->fw_version.sensor_id);
	} else if (ts_core->lockdown_info[1] == 0x42) {  //TM
...
static void goodix_shortcircut_test(struct goodix_ts_test *ts_test)
{
	...
	if (ret < 0) {
		ts_err("Failed enter short test mode");
    		...
    		xiaomi_touch_mievent_report_str(TOUCH_EVENT_SHORTTEST_FAIL, 0, "TpShortTestFail", "goodix");
#endif
		return;
	}
...
static void goodix_capacitance_test(struct goodix_ts_test *ts_test)
{
	...
	if (ret < 0) {
		...
		if (ret < 0) {
#if IS_ENABLED(CONFIG_MIEV)
			xiaomi_touch_mievent_report_str(TOUCH_EVENT_OPENTEST_FAIL, 0, "TpOpenTestFail", "goodix");
#endif
...
int goodix_factory_test(void)
{
	int result = 0;
	if ((rawdata_test_result == 1) && (deltadata_result == 1) &&
   			(noisedata_result == 1) && (short_result ==1)) {
		ts_info("********************ALL test pass!**********************");
		result = 1;
	}else {
		ts_info("******************** Test fail!**********************");
		result = 0;
...
static int proc_ctp_openshort_show(struct seq_file *m, void *v)
{
	struct goodix_ts_core *cd = m->private;

	goodix_openshort_test(cd);
	if (goodix_factory_test() == 0)
		seq_printf(m, "%s\n", "result=0");
	else
		seq_printf(m, "%s\n", "result=1");
...
{
	return single_open_size(file, proc_ctp_openshort_show,
			PDE_DATA(inode), DEFAULT_SEQ_FILE_SIZE);
}
...
int proc_node_init(struct goodix_ts_core *core_data)
{
	...
	if (proc_touchscreen_dir == NULL) {
		ts_info(KERN_ERR "%s: proc_touchpanel_dir file create failed!\n",
		       __func__);
	} else {
		proc_ctp_openshort_test_file = proc_create_data("ctp_openshort_test",
			0666, proc_touchscreen_dir, &proc_ctp_openshort_test_fops, core_data);
		if (proc_ctp_openshort_test_file == NULL) {
			ts_info(KERN_ERR "%s: ctp_openshort_test create failed!\n", __func__);
			remove_proc_entry("touchscreen", NULL);
		} else {
			ts_info("%s:  proc_create ctp_openshort_test success", __func__);
		}
	}
