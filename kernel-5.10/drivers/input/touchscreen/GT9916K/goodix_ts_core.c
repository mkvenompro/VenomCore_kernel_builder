	...
	struct goodix_ts_board_data *board_data)
{
	...
	if (chosen) {
		...
		if (tmp_size > 0) {
			sscanf(tmp_buf, "%02hhx", &(board_data->panel_soft_id));
			ts_info("[%s]: lcm_soft_id = %s, size = %d\n", __func__, tmp_buf, tmp_size);
		} else {
			ts_info("[%s]:get lcm_soft_id failed\n", __func__);
		}
	} else {
		ts_info("[%s]:find chosen failed\n", __func__);
	}

	if (strstr(tmp_buf,"30")) {
		fw_db_num = 1;
		ts_info("VOIX old CG 30");
	} else if (strstr(tmp_buf,"31")) {
		fw_db_num = 2;
		ts_info("VOIX new CG 31");
	}
	...
	if(goodix_panel_name == 1) {
		if (fw_db_num == 1)
			name_tmp = "goodix_firmware_16.bin";
		else if (fw_db_num == 2)
			name_tmp = "goodix_firmware_16_cg.bin";
	}
	...
	if(goodix_panel_name == 1) {
		if (fw_db_num == 1)
			...
			name_tmp = "goodix_cfg_group_16_cg.bin";
	}
...
static int goodix_later_init_thread(void *data)
{
	...
	if (ret) {
		...
		xiaomi_touch_mievent_report_str(TOUCH_EVENT_FWLOAD_ERR, 0, "TpFirmwareLoadFail", "goodix");
#endif
...
int goodix_ts_get_lockdown_info(struct goodix_ts_core *cd)
{
	...
	if (ret) {
		ts_err("can't get lockdown, use default!");
		cd->lockdown_info[0] = 0x46;
		cd->lockdown_info[1] = 0x36;
		cd->lockdown_info[2] = 0x32;
		cd->lockdown_info[3] = 0x10;
		cd->lockdown_info[4] = 0x50;
		cd->lockdown_info[5] = 0x37;
		cd->lockdown_info[6] = 0x32;
		cd->lockdown_info[7] = 0x00;
	}
...
int goodix_set_cur_value(int gtp_mode, int gtp_value)
{
	int ret = 0;
    ...
    */
	...
	if (gtp_mode <= Touch_Panel_Orientation) {/*power state no need call game work*/
		if (gtp_mode ==  Touch_Game_Mode && goodix_core_data && gtp_value >= 0) {
			...
			if (goodix_core_data->gamemode_enabled == true) {
				cmd.cmd = 0xC1;
				cmd.len = 6;
				cmd.data[0] = 0x01;
				if (cd->hw_ops->send_cmd(cd, &cmd)) {
					ts_info("open 480Hz cmd in gamemode fail, data0 = %d", cmd.data[0]);
					mutex_unlock(&cd->edge_data_mutex);
					return -EINVAL;
				} else {
					ts_info("open 480Hz cmd in gamemode suss, data0 = %d", cmd.data[0]);
				}
			} else if (goodix_core_data->gamemode_enabled == false) {
				cmd.cmd = 0xC1;
				cmd.len = 6;
				cmd.data[0] = 0x00;
				if (cd->hw_ops->send_cmd(cd, &cmd)) {
					ts_info("colse 480Hz cmd in normal fail, data0 = %d", cmd.data[0]);
					mutex_unlock(&cd->edge_data_mutex);
					return -EINVAL;
				} else {
					ts_info("colse 480Hz cmd in normal suss, data0 = %d", cmd.data[0]);
				}
			}
			mutex_unlock(&cd->edge_data_mutex);
		}	
...
static ssize_t goodix_fodpress_write(struct file *file, const char __user *buf,
		size_t count, loff_t *pos)
{
	struct goodix_ts_hw_ops *hw_ops;
	int retval = 0;
	...
	if (!goodix_core_data)
		return GTP_RESULT_INVALID;
	hw_ops = goodix_core_data->hw_ops;
	if (!strncmp("open", tmp, 4)) {
		retval = goodix_set_cur_value(10,1);
		ts_info("factory mode open fod mode!");
	} else if (!strncmp("close", tmp, 5)) {
		retval = goodix_set_cur_value(10,0);
		ts_info("factory mode close fod mode!");
	}
	goodix_core_data->result_type = retval;
out:
	if (retval >= 0)
...
static const struct proc_ops goodix_fodpress_ops = {
	.proc_write = goodix_fodpress_write,
	.proc_lseek = default_llseek,
};
...
static void goodix_touch_dfs_test(int value)
{
    switch(value){
        ...
        case TOUCH_EVENT_OPENTEST_FAIL:
            xiaomi_touch_mievent_report_int(TOUCH_EVENT_OPENTEST_FAIL, 0, "TpOpenTestFail", "goodix", 0);
            break;
        case TOUCH_EVENT_SHORTTEST_FAIL:
            xiaomi_touch_mievent_report_int(TOUCH_EVENT_SHORTTEST_FAIL, 0, "TpShortTestFail", "goodix", 0);
            break;
        default:
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	core_data->tp_fod_press_proc =
		proc_create("tp_fod_press", 0664, NULL, &goodix_fodpress_ops);
#ifdef GOODIX_DEBUGFS_ENABLE
...
#if IS_ENABLED(CONFIG_MIEV)
	xiaomi_touch_interfaces.touch_dfs_test = goodix_touch_dfs_test;
#endif
...
static int __init goodix_ts_core_init(void)
{
	...
	if(goodix_focal_panel_name() == 1) {
		goodix_panel_name = 1;
		ts_info("TP is goodix 9916k VIOX");
	} else {
		ts_info("goodix 9916k no match,exit");
		return 0;
	}
