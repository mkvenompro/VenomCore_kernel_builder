...
static int bl_lhbm_threshold =277;
...
static const char *panel_name = "panel_name=dsi_p7_35_0f_0c_dsc_vdo";
...
static struct LCM_setting_table local_hbm_normal_white_1200nit[] = {
	{0x9F, 01, {0x04}},
	{0xBE, 17, {0x31, 0x2A, 0x5F, 0x3D, 0xEB, 0x2A, 0x5F, 0x3D, 0xEB, 0x2A, 0x63, 0x3F, 0xDF, 0x2A, 0x6B, 0x43, 0xE6}},
	{0x97, 04, {0x11, 0xAF, 0xFF, 0x00}},
};

static struct LCM_setting_table local_hbm_normal_white_200nit[] = {
	{0x9F, 01, {0x04}},
	{0xBE, 17, {0x31, 0x16, 0xD5, 0xBC, 0x2E, 0x16, 0xD7, 0xBF, 0x31, 0x16, 0xDA, 0xC2, 0x34, 0x16, 0xDF, 0xC9, 0x3A}},
	{0x97, 04, {0x11, 0xAF, 0xFF, 0x00}},
};
	...
	if (chosen) {
		...
		if (tmp_size > 0) {
			...
			if ((tmp_size-1) < 68) {
				...
				// 转换前17个字节到1200nit
				for (i = 0; i < 17; i++) {
					sscanf(tmp_buf + i*2, "%2hhx", &local_hbm_normal_white_1200nit[1].para_list[i]);
				}
				// 转换接下来的17个字节到200nit
				for (i = 0; i < 17; i++) {
					sscanf(tmp_buf + 34 + i*2, "%2hhx", &local_hbm_normal_white_200nit[1].para_list[i]);
				}
			}
		}
	} else {
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0xc5, 0x09, 0x20, 0x88, 0x41, 0x99, 0x80, 0x19, 0x00, 0x4e, 0x57, 0x39, 0x3a, 0x3a, 0x2a, 0x0a, 0x2f, 0x00, 0x20, 0x0f);
	lcm_dcs_write_seq_static(ctx, 0x9f, 0x05);
	lcm_dcs_write_seq_static(ctx, 0xb4, 0x02, 0x0e, 0x3e, 0x00, 0x05, 0x10, 0x06, 0x00, 0x00, 0x02, 0x60, 0x8d);
	...
	lcm_dcs_write_seq_static(ctx, 0xee, 0x0D, 0x00, 0x00, 0x00, 0x08, 0x88, 0x08, 0x08, 0x00, 0x30, 0x88, 0x88, 0x29, 0x00, 0x00, 0x00, 0x00);
	lcm_dcs_write_seq_static(ctx, 0x9f, 0x01);
	lcm_dcs_write_seq_static(ctx, 0xb4, 0x10, 0x00, 0x00, 0x03);
	lcm_dcs_write_seq_static(ctx, 0xe4, 0x00, 0x00, 0x80, 0x0c, 0xff, 0x0f, 0x08, 0x08);
	lcm_dcs_write_seq_static(ctx, 0xea, 0x04, 0x00);
	lcm_dcs_write_seq_static(ctx, 0x9f, 0x01);
	lcm_dcs_write_seq_static(ctx, 0xcd, 0x0B, 0x1F, 0x1F, 0xF0, 0x03, 0x88, 0x00, 0x96, 0xF0, 0x0A, 0x68, 0x00, 0x96, 0x1F, 0x00, 0x68, 0x00, 0x96, 0x1F, 0x0A, 0x68, 0x00, 0x96);
	lcm_dcs_write_seq_static(ctx, 0xca, 0x05, 0x01, 0x00, 0x01, 0x01, 0x03, 0x2e, 0x02, 0x03, 0x03, 0x2e, 0x02, 0x03, 0x03, 0x2e, 0x02, 0x03, 0x88, 0xaa, 0x81);
	lcm_dcs_write_seq_static(ctx, 0xcb, 0x01, 0x01, 0x01, 0x03, 0x2e, 0x02, 0x03, 0x03, 0x2e, 0x02, 0x03, 0x03, 0x2e, 0x02, 0x03);
	...
	lcm_dcs_write_seq_static(ctx, 0x9f, 0x04);
	lcm_dcs_write_seq_static(ctx, 0xc1, 0x1F, 0xFE, 0x1A, 0x9E, 0x15, 0x4E, 0x0F, 0xFE, 0x05, 0x5C, 0x02, 0x2A, 0x02, 0x28, 0x00, 0xD4, 0x00, 0x72, 0x00, 0x32, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
	lcm_dcs_write_seq_static(ctx, 0x9f, 0x05);
	lcm_dcs_write_seq_static(ctx, 0xb5, 0x1F, 0xFE, 0x1A, 0x9E, 0x15, 0x4E, 0x0F, 0xFE, 0x05, 0x5C, 0x02, 0x2A, 0x02, 0x28, 0x00, 0xD4, 0x00, 0x72, 0x00, 0x32);
	lcm_dcs_write_seq_static(ctx, 0x9f, 0x07);
	lcm_dcs_write_seq_static(ctx, 0xd6, 0x1F, 0xFE, 0x1A, 0x9E, 0x15, 0x4E, 0x0F, 0xFE, 0x05, 0x5C, 0x02, 0x2A, 0x02, 0x28, 0x00, 0xD4, 0x00, 0x72, 0x00, 0x32, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
	lcm_dcs_write_seq_static(ctx, 0x9f, 0x09);
	lcm_dcs_write_seq_static(ctx, 0xb4, 0xFD, 0xFF, 0x4F, 0xA7, 0xA7, 0xFF, 0x21, 0xAE, 0x15, 0x10, 0x14, 0x6A, 0x00, 0x39, 0x19);
	lcm_dcs_write_seq_static(ctx, 0x9f, 0x08);
	lcm_dcs_write_seq_static(ctx, 0xcc, 0x00, 0x20, 0x16, 0x0B);
	lcm_dcs_write_seq_static(ctx, 0xd6, 0xFE, 0x00, 0x23, 0x07, 0xF3, 0x57, 0xD1, 0x9E, 0x9B, 0x63, 0x14, 0xCE, 0xB1, 0x2D, 0xF1, 0xB9, 0x90, 0x11, 0x90, 0x90, 0x11, 0x90, 0x90,
		0x11, 0x90, 0x90, 0xFE, 0x00, 0x13, 0xFE, 0xE2, 0x57, 0xB7, 0x7F, 0x9A, 0x39, 0xDB, 0xCD, 0x75, 0xDE, 0xF0, 0x46, 0xE4, 0x00, 0xE4, 0xE4, 0x00, 0xE4, 0xE4, 0x00, 0xE4, 0xE4);
	lcm_dcs_write_seq_static(ctx, 0xd7, 0xFE, 0x00, 0x13, 0xFF, 0xEA, 0x57, 0xC3, 0x96, 0x9B, 0x57, 0x09, 0xCE, 0xB0, 0x17, 0xF1, 0x77, 0x1A, 0x11, 0x1A, 0x1A, 0x11, 0x1A, 0x1A, 0x11, 0x1A, 0x1A);
...
static char bl_tb2[] = { 0xB6, 0x05, 0x00, 0x00, 0x11, 0x11, 0x11, 0x2C, 0x08, 0x1C, 0x21, 0x21, 0x21, 0x21, 0x21, 0x21};
static void boe_get_bl_tb2(unsigned int level)
{
	unsigned int DBV_Star=0;
	unsigned int DBV_End=0;
	unsigned char left=1;
	unsigned char right=boe_dbv_vint_buf[5]/6;
	unsigned char mid=0;
	...
	{
		mid=(left+right)/2;
		DBV_Star=(boe_dbv_vint_buf[6*mid]<<4)|((boe_dbv_vint_buf[6*mid+1]&0xF0)>>4);
		DBV_End=((boe_dbv_vint_buf[6*mid+1]&0x0F)<<8)|boe_dbv_vint_buf[6*mid+2];
		count_step++;
		...
		{
			pr_info(" mid=%d\n",mid);
			bl_tb2[ARRAY_SIZE(bl_tb2)-1]=boe_dbv_vint_buf[6*mid+3];
			bl_tb2[ARRAY_SIZE(bl_tb2)-3]=boe_dbv_vint_buf[6*mid+4];
			bl_tb2[ARRAY_SIZE(bl_tb2)-5]=boe_dbv_vint_buf[6*mid+5];
			pr_info("60hz bl_tb = %x, 90hz bl_tb= %x, 120hz bl_tb= %x\n", bl_tb2[ARRAY_SIZE(bl_tb2)-1],
...
#define BL_APL_THRESHOLD 3407
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb,
	void *handle, unsigned int level)
{
	static int apl_on = 0;
	char bl_tb0[] = {0x51, 0x00, 0x00};
	char bl_tb1[] = { 0x9F, 0x02};
	char bl_tb3[] = { 0x9F, 0x08};
	char bl_tb4[] = { 0xCC, 0x00, 0x20, 0x58};
	...
	if (fod_in_calibration || lhbm_flag || ctx->dynamic_fps == 30) {
		...
		boe_get_bl_tb2(level);
		if ((level <= BL_APL_THRESHOLD) && apl_on) {
			bl_tb4[2] = 0x20;
			bl_tb4[3] = 0x58;
			mutex_lock(&ctx->panel_lock);
			...
			mutex_unlock(&ctx->panel_lock);
			apl_on = 0;
		} else if (level > BL_APL_THRESHOLD && level <= 4095) {
			bl_tb4[1] = 0x01;
			bl_tb4[3] = calculate_apl_gain(level); //Math.floor((DBV-3407)/(4095-3407)*(22-88)+88)
			pr_info(" level=%d,bl_tb4[1]=%d,bl_tb4[3]=%d\n",level, bl_tb4[1], bl_tb4[3]);
			mutex_lock(&ctx->panel_lock);
			cb(dsi, handle, bl_tb3, ARRAY_SIZE(bl_tb3));
			cb(dsi, handle, bl_tb4, ARRAY_SIZE(bl_tb4));
			mutex_unlock(&ctx->panel_lock);
...
static struct mtk_panel_params ext_params = { /* 60HZ */
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[2] = {30, 2, {0x90, 0x00} },
		.dfps_cmd_table[3] = {30, 2, {0x9f, 0x0c} },
		.dfps_cmd_table[4] = {30, 9, {0xb4, 0xe1, 0xc0, 0x88, 0x1b, 0x1b, 0x95, 0x4e, 0x18} },
	},
...
static int mi_disp_panel_update_lhbm_reg(struct mtk_dsi * dsi, enum lhbm_cmd_type type, int bl_level)
{
	...
	if (bl_level >= 0) {
		bl_tb[0] = (bl_level >> 8) & 0xFF;
		bl_tb[1] = bl_level & 0xFF;
		if (bl_level < bl_lhbm_threshold) {
			alpha_buf[0] = (boe_alpha_set[bl_level] >> 8) & 0xff;
			alpha_buf[1] = boe_alpha_set[bl_level] & 0xff;
			local_hbm_normal_white_1200nit[2].para_list[1] = alpha_buf[0];
			local_hbm_normal_white_1200nit[2].para_list[2] = alpha_buf[1];
			local_hbm_normal_white_200nit[2].para_list[1] = alpha_buf[0];
			local_hbm_normal_white_200nit[2].para_list[2] = alpha_buf[1];
		} else {
			local_hbm_normal_white_1200nit[2].para_list[1] = 0xAF;
			local_hbm_normal_white_1200nit[2].para_list[2] = 0xFF;
			local_hbm_normal_white_200nit[2].para_list[1] = 0xAF;
			local_hbm_normal_white_200nit[2].para_list[2] = 0xFF;
		}
...
static struct mtk_panel_params ext_params_mode_3 = { /* 30HZ */
	...
	.dyn_fps = {
		.switch_en = 1,
		.vact_timing_fps = 30,
		.dfps_cmd_table[0] = {0, 3, {0x51, 0x0f, 0xff} },
		.dfps_cmd_table[1] = {0, 2, {0x90, 0x01} },
		.dfps_cmd_table[2] = {0, 2, {0x9f, 0x0c} },
		.dfps_cmd_table[3] = {0, 9, {0xb4, 0xf1, 0xc0, 0x0b, 0x1b, 0x1b, 0x98, 0x6d, 0x16} },
	},
	.physical_width_um = PHYSICAL_WIDTH,
	.physical_height_um = PHYSICAL_HEIGHT,
};
				   ...
				   unsigned int mode)
{
	...
	if (drm_mode_vrefresh(m) == MODE_0_FPS) {
		...
		if (ctx->doze_brightness == DOZE_BRIGHTNESS_HBM) {
			ext_params_mode_3.dyn_fps.dfps_cmd_table[0].para_list[1] = 0x0f;
			ext_params_mode_3.dyn_fps.dfps_cmd_table[0].para_list[2] = 0xff;
		} else if (ctx->doze_brightness == DOZE_BRIGHTNESS_LBM) {
			ext_params_mode_3.dyn_fps.dfps_cmd_table[0].para_list[1] = 0x01;
			ext_params_mode_3.dyn_fps.dfps_cmd_table[0].para_list[2] = 0x55;
		}
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "p7_35_0f_0c_dsc_vdo,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-p7-35-0f-0c-dsc-vdo",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("panel-p7-35-0f-0c-dsc-vdo Panel Driver");
