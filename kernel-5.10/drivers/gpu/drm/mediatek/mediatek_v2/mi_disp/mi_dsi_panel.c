...
#define DEFAULT_MAX_BRIGHTNESS_CLONE 16383
...
ssize_t mi_dsi_panel_write_mipi_reg(char *buf)
{
	...
	DISP_INFO("mi_dsi_panel_write_mipi_reg+++:input buffer:{%s}\n", buf);
	...
	if (lcm_mipi_read_write.read_enable) {
		...
		if (lcm_mipi_read_write.read_count > 10) {
			...
			pr_err("read lcm addr:%pad--dlen:%d\n",
				&(*(char *)(cmd_msg->tx_buf[0])), (int)cmd_msg->rx_len[0]);
			for (j = 0; j < cmd_msg->rx_len[0]; j++) {
				pr_err("read lcm addr:%pad--byte:%d,val:%pad\n",
					&(*(char *)(cmd_msg->tx_buf[0])), j,
...
ssize_t  mi_dsi_panel_read_mipi_reg(char *buf)
{
	...
	const char help_message[] = {
		...
		"\tExample:\n"
		"\t\twrite:00 00 39 01 00 00 00 00 03 51 07 FF\n"
		"\t\tread: 01 02 06 01 00 01 00 00 01 52\n"
	};
...
int mi_dsi_panel_get_panel_soft_id(struct mtk_dsi *dsi, char *buf)
{
	struct mtk_ddp_comp *comp =  NULL;
	struct mtk_panel_ext *panel_ext = NULL;
	...
	if (!(panel_ext && panel_ext->funcs && panel_ext->funcs->get_panel_soft_id)) {
		pr_info("%s get_panel_soft id func not defined", __func__);
		return 0;
	} else {
		return panel_ext->funcs->get_panel_soft_id(dsi->panel, buf);
	}
}
			...
			int backlight)
{
	...
	DISP_ERROR("%s ,  backlight = %d\n", __func__, backlight);
	mtk_crtc = dsi->ddp_comp.mtk_crtc;
	mtk_drm_setbacklight(&mtk_crtc->base, backlight);
	return ret;
}
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
	...
	pr_err("panel feature: %s, value: %d\n",
		get_disp_feature_id_name(ctl->feature_id), ctl->feature_val);
	...
	case DISP_FEATURE_ROUND_MODE:
		DISP_INFO("round mode:%x\n", ctl->feature_val);
		if (mi_cfg->feature_val[DISP_FEATURE_ROUND_MODE] == ctl->feature_val) {
			DISP_INFO("round mode is the same, return\n");
			break;
		}
		if (panel_ext->funcs->panel_set_round_enable) {
			panel_ext->funcs->panel_set_round_enable(dsi->panel, ctl->feature_val);
		}
		mi_cfg->feature_val[DISP_FEATURE_ROUND_MODE] = ctl->feature_val;
		break;
	default:
...
void mi_disp_cfg_init(struct mtk_dsi *dsi)
{
	...
	mi_cfg->feature_val[DISP_FEATURE_ROUND_MODE] = 1;
