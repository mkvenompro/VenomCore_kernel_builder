...
static ssize_t panel_soft_id_show(struct device *device,
			   struct device_attribute *attr,
			   char *buf)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		return mi_dsi_display_read_panel_soft_id(dd_ptr->display, buf);
	} else {
		return snprintf(buf, PAGE_SIZE, "Unsupported display(%s intf)\n",
			get_disp_intf_type_name(dd_ptr->intf_type));
	}
			   ...
			   const char *buf, size_t count)
{
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		...
		if (doze_brightness == DOZE_BRIGHTNESS_HBM || doze_brightness == DOZE_BRIGHTNESS_LBM)
			((struct mtk_dsi *)(dd_ptr->display))->doze_enabled = true;
		else
			((struct mtk_dsi *)(dd_ptr->display))->doze_enabled = false;
...
static DEVICE_ATTR_RO(panel_soft_id);
