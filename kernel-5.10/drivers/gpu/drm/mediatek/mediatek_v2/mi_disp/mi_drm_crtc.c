...
static int mi_drm_update_fod_anim_status(struct mtk_dsi *dsi, bool fod_anim_flag)
{
	...
	mi_cfg = &dsi->mi_cfg;
	if (mi_cfg->fod_anim_flag == false && fod_anim_flag == true && dsi->mi_cfg.doze_state) {
		DISP_INFO("exit aod\n");
		...
		dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] =
			DOZE_TO_NORMAL;
	} else if (mi_cfg->fod_anim_flag == true && fod_anim_flag == false && dsi->mi_cfg.doze_state && dsi->mi_cfg.user_doze_brightness) {
		DISP_INFO("enter aod\n");
		mi_disp_feature_event_notify_by_type(
			mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE,
