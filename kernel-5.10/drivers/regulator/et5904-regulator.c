			      ...
			      int max_uV, unsigned *selector)
{
	...
	if(ET5904_first_supply == ldo_supply)
	{
...
static int et5904_voltage_get(struct regulator_dev *rdev)
{
	...
	{
		if (id == ET5904_ID_REGULATOR1 || id == ET5904_ID_REGULATOR2) {
			return val * 8000 + 504000;
		} else {
			return val * 8000 + 1384000;
		}
	}
...
static int et5904_regulator_init(struct et5904_regulator *chip)
{
	...
	ret = regmap_write(chip->regmap, ET5904_REG_LDO_EN, 0x01);
	if (ret < 0) {
			    ...
			    const struct i2c_device_id *id)
{
	...
	dev_err(chip->dev, "chip id = 0x%x.\n", data);
	switch (data) {
	case AW37004D_DEVICE_ID:
		chip->chip_id = ET5904;
		ldo_supply = ET5904_first_supply;
		ET_DBG("Third party ldo chip_id AW37004DNR");
		break;
	case SGM38121_DEVICE_ID:
		chip->chip_id = ET5904;
		ldo_supply = SGM38121_second_supply;
		ET_DBG("Third party ldo chip_id SGM38121");
		break;
	default:
