...
static int tcpci_vbus_level_changed(struct tcpc_device *tcpc)
{
	...
	TCPC_INFO("ps_change=%d, chip_id=0x%x\n", tcpc->vbus_level, chip_id); 
...
static int tcpci_alert_recv_msg(struct tcpc_device *tcpc)
{
	...
	if ((!rv1 && (SC2150_PID == chip_pid) && !in_bist_mode) || tcpc->bist_mode_ctrl) {
		TCPC_INFO("slw close rx\n");
...
out:
	tcpci_get_chip_id(tcpc, &chip_id);
	if ((NU6601_DID_A != chip_id) || tcpc->bist_mode_ctrl) {
	...
	if (!in_bist_mode) {
		...
		if ((!rv1 && (SC2150_PID == chip_pid)) || tcpc->bist_mode_ctrl) {
			TCPC_INFO("slw open rx\n");
