...
#include "s5khm9_aac_main_mipi_raw_Sensor.h"
#include "s5khm9_aac_setting.h"
#undef VENDOR_EDIT
#define PFX "S5KHM9_AAC_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHM9_AAC_MAIN_SENSOR_ID,	//0x1AD9 +1 
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = read_cmos_sensor(0x0000) + 1;
			pr_err("s5khm9-aac sensor id = 0x%x, vendor id = 0x%x",*sensor_id ,vendor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_err("s5khm9-aac i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				if(vendor_id == VENDOR_ID){
					return ERROR_NONE;
				}
				else{
					pr_err("s5khm9-aac read vendor id fail");
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
			}
			pr_err("s5khm9-aac Read sensor id fail, sensor id: 0x%x, i2c: 0x%x\n", *sensor_id, imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
UINT32 S5KHM9_AAC_MAIN_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
