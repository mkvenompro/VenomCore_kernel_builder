...
#include "sc202v_aac_mipi_raw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "sc202v_aac_mipi_raw"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)
#define LOG_1 LOG_INF("SC202V,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
#define VENDOR_ID 0x196

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = SC202V_AAC_DEPTH_SENSOR_ID,
...
{
	return (((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108))+1); //0xeb52
}
...
static kal_uint16 sc202v_read_otp(void)
{
    ...
    for(i = 0; i < ARRAY_SIZE(otpData); i++){
        LOG_INF("depth-aac read fuseid 0x%x", otpData[i]);
    }
    return ret;
}

unsigned int sc202v_aac_read_otp_info(struct i2c_client *client,
...
EXPORT_SYMBOL(sc202v_aac_read_otp_info);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_err("depth-aac get imgsensorid!!!\n");
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            pr_err("depth-aac return_sensor_id:0x%x\n",*sensor_id);
...
UINT32 SC202V_AAC_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SC202V_AAC_MIPI_RAW_SensorInit    */
