 ...
 * Filename:
 * ---------
 *     sc202v_sunny_mipi_raw_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "sc202v_sunny_mipi_raw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "sc202v_sunny_mipi_raw"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)
#define LOG_1 LOG_INF("SC202V,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
#define VENDOR_ID 0x195

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = SC202V_SUNNY_DEPTH_SENSOR_ID,
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_MONO,    //+bug682590,zhanghengyuan.wt,ADD,2021/8/26,fix depth sc202v output_dataformat
...
//#ifdef __GAINMAP__
#if 1
#define SC202V_SENSOR_GAIN_BASE             1024
#define SC202V_SENSOR_GAIN_MAX              65024 //(63.5 * SC202V_SENSOR_GAIN_BASE)
#define SC202V_SENSOR_GAIN_MAX_VALID_INDEX  6
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	kal_uint16 reg_gain = gain << 4;

	if (reg_gain < SC202V_SENSOR_GAIN_BASE)
		reg_gain = SC202V_SENSOR_GAIN_BASE;
	else if (reg_gain > SC202V_SENSOR_GAIN_MAX)
		reg_gain = SC202V_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC202V_AGC_Param[SC202V_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	for (gain_index = SC202V_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 1; gain_index--)
		if (reg_gain >= SC202V_AGC_Param[gain_index][0])
			break;
	write_cmos_sensor(0x3e09, SC202V_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC202V_SENSOR_GAIN_BASE / SC202V_AGC_Param[gain_index][0];
...
{
	LOG_INF("depth-sunny sensor_init");
...
*
*************************************************************************/
static char otpData[8]={0};
static kal_uint16 vendor_id =0;
static kal_int8 sc202v_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
{
    int i = 0;
    ...
    write_cmos_sensor(0x4409, 0x11);
    write_cmos_sensor(0x440b, 0x16);
    write_cmos_sensor(0x0100, 0x01);
    write_cmos_sensor(0x4400, 0x11);
    vendor_id = (read_cmos_sensor(0x8011) << 8) | read_cmos_sensor(0x8012);
    for (i = 0; i < addr_len; i++) {
        result[i] = read_cmos_sensor(addr[i]);
        LOG_INF("depth-sunny read result[%d] is %x", i, result[i]);
    }
    return 0;
}

static kal_uint16 sc202v_read_otp(void)
{
    int ret = 0;
    kal_uint8 i;
    int otpdata_addr[] = {0x8000, 0x8001,0x8002,0x8003, 0x8004, 0x8005, 0x8006, 0x8007};
    ret = sc202v_read_general_info(otpdata_addr, ARRAY_SIZE(otpdata_addr), otpData);
    if (ret != 0) {
        LOG_INF("depth-sunny read fuseid failed\n");
        return -1;
    }
    for(i = 0; i < ARRAY_SIZE(otpData); i++){
        LOG_INF("depth-sunny read otp data 0x%x", otpData[i]);
    }
    return ret;
}

unsigned int sc202v_sunny_read_otp_info(struct i2c_client *client,
        unsigned int addr, unsigned char *data, unsigned int size)
{
    int num = ARRAY_SIZE(otpData);
    int index = 0;
    LOG_INF(" depth-sunny addr:0x%x ,size:%d",addr, size);
    if(size == 2){
        data[0] = (vendor_id >> 0) & 0xff;
        data[1] = (vendor_id >> 8) & 0xff;
        return 2;
    }else{
        for(;index<num ;index++){
...
EXPORT_SYMBOL(sc202v_sunny_read_otp_info);

static kal_uint16 get_vendor_id(void)
{
    LOG_INF("depth-aac vendor_id 0x%x", vendor_id);
    return vendor_id;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_err("depth-sunny get imgsensorid!!!\n");
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            pr_err("depth-sunny return_sensor_id:0x%x\n",*sensor_id);
	...
	kal_uint8 i = 0;
	 ...
	 while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		 ...
		 do {
			 sensor_id = return_sensor_id();
			 if (sensor_id == imgsensor_info.sensor_id) {				 
				 LOG_INF("depth-sunny i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);   
				 break;
			 }	 
			 LOG_INF("depth-sunny Read sensor id fail, return_sensor_id: 0x%x\n",sensor_id);
			 retry--;
		 } while(retry > 0);
		 i++;
		 if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 SC202V_SUNNY_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SC202V_SUNNY_MIPI_RAW_SensorInit    */
