...
#define AW87XXX_DRIVER_VERSION	"v4.0.3"
...
int aw87xxx_update_profile(struct aw87xxx *aw87xxx, char *profile)
{
	...
	len = strlen(profile) <= strlen(aw87xxx->current_profile) ? strlen(profile):strlen(aw87xxx->current_profile);

	if((strncmp(profile, aw87xxx->current_profile, len) == 0)) {
		AW_DEV_LOGD(aw87xxx->dev, "profile[%s] is already active!", profile);
		return 0;
	}
