...
#define OCS_CODE_VERSION "V5.3.0"

#define DEBUG_LOG_LEVEL

#ifdef DEBUG_LOG_LEVEL
#define DBG(fmt, arg...) \
	pr_debug("OCS_BIN %s,line= %d,"fmt, __func__, __LINE__, ##arg)

#define DBG_ERR(fmt, arg...) \
	pr_err("OCS_BIN_ERR %s,line= %d,"fmt, __func__, __LINE__, ##arg)
...
static int oca_check_sum(struct oca_bin *bin, int bin_num)
{
	...
	DBG("bin_num=%d, check_sum = 0x%x, sum_data = 0x%x\n", bin_num, check_sum, sum_data);
	if (sum_data != check_sum) {
		p_check_sum = NULL;
		DBG_ERR("check sum or check bin data len error\n");
		DBG_ERR("bin_num=%d, check_sum = 0x%x, sum_data = 0x%x\n", bin_num, check_sum, sum_data);
		return -EINVAL;
	}
	p_check_sum = NULL;
...
static int oca_check_data_version(struct oca_bin *bin, int bin_num)
{
	...
	DBG_ERR("Unrecognized this bin data version\n");
	return -EINVAL;
}
...
static int oca_check_register_num_v1(struct oca_bin *bin, int bin_num)
{
	...
	DBG
	    ("bin_num=%d, parse_register_num = 0x%x, check_register_num = 0x%x\n",
	     bin_num, parse_register_num, check_register_num);
	if (parse_register_num != check_register_num) {
		p_check_sum = NULL;
		DBG_ERR("register num is error\n");
		DBG_ERR("bin_num=%d, parse_register_num = 0x%x, check_register_num = 0x%x\n", bin_num, parse_register_num, check_register_num);
		return -EINVAL;
	}
...
static int oca_check_dsp_reg_num_v1(struct oca_bin *bin, int bin_num)
{
	...
	DBG
	    ("bin_num=%d, parse_dsp_reg_num = 0x%x, check_dsp_reg_num = 0x%x\n",
	     bin_num, parse_dsp_reg_num, check_dsp_reg_num);
	if (parse_dsp_reg_num != check_dsp_reg_num) {
		p_check_sum = NULL;
		DBG_ERR("dsp reg num is error\n");
		DBG_ERR("bin_num=%d, parse_dsp_reg_num = 0x%x, check_dsp_reg_num = 0x%x\n", bin_num, parse_dsp_reg_num, check_dsp_reg_num);
		return -EINVAL;
	}
...
static int oca_check_soc_app_num_v1(struct oca_bin *bin, int bin_num)
{
	...
	DBG
	    ("bin_num=%d, parse_soc_app_num = 0x%x, check_soc_app_num = 0x%x\n",
	     bin_num, parse_soc_app_num, check_soc_app_num);
	if (parse_soc_app_num != check_soc_app_num) {
		p_check_sum = NULL;
		DBG_ERR("soc app num is error\n");
		DBG_ERR("bin_num=%d, parse_soc_app_num = 0x%x, check_soc_app_num = 0x%x\n", bin_num, parse_soc_app_num, check_soc_app_num);
		return -EINVAL;
	}
...
static int oca_parse_bin_header_1_0_0(struct oca_bin *bin)
{
	...
	case DATA_TYPE_MULTI_BINS:
		...
		break;
	default:
		DBG_ERR("Unrecognized this bin data type\n");
		break;
	}
	return 0;
}
...
static int oca_check_bin_header_version(struct oca_bin *bin)
{
	...
	case HEADER_VERSION_1_0_0:
		...
		return ret;
	default:
		DBG_ERR("Unrecognized this bin header version \n");
		return -EINVAL;
	}
}
...
int oca72xxx_parsing_bin_file(struct oca_bin *bin)
{
	...
	if (ret < 0) {
		DBG_ERR("check bin header version error\n");
		return ret;
	}
	bin->p_addr = NULL;
	...
	for (i = 0; i < bin->all_bin_parse_num; i++) {
		...
		if (ret < 0) {
			DBG_ERR("check sum data error\n");
			return ret;
		}
		/* check bin data version */
		ret = oca_check_data_version(bin, i);
		if (ret < 0) {
			DBG_ERR("check data version error\n");
			return ret;
		}
		/* check valid data */
